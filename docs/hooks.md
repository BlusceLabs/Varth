...
Varth supports Pi-native extension package hooks and user-owned Bash hooks.
...
```bash
varth install npm:<package-name>
varth list
```

Pi package extensions subscribe to native Pi events such as `tool_call`, `tool_result`, `session_start`, `session_before_compact`, `session_compact`, and `session_shutdown`. Varth includes a narrow Pi compatibility shim for packages that expect older field names, including `tool_result.output`, `tool_result.params`, and the legacy `before_provider_response` event name.

## Core Bash Hooks

Varth core still supports local Bash command hooks for commands executed through the `bash` tool and interactive `!` / `!!` shell commands.

Global hooks:

```bash
~/.config/varth/harness/hooks/bash/*.sh
~/.config/varth/harness/hooks/bash/*.bash
```
...
```bash
varth resources list
varth resources disable hooks.bash
varth resources enable hooks.bash.project.my-hook-sh
varth resources disable hooks.bash.global.my-hook-sh
```

Each Bash hook runs as `bash <hook-path>`. Varth passes the current command in environment variables:

```bash
VARTH_HOOK_EVENT=tool_call
VARTH_TOOL_NAME=bash
VARTH_TOOL_INPUT_COMMAND='git status'
CRUSH_TOOL_INPUT_COMMAND='git status'
```

Varth also writes JSON to stdin:
...
Claude Code hook format is not part of the varth core hook contract. Varth can run existing Claude Code command hooks through a disabled-by-default compatibility extension.

Enable the adapter:

```bash
varth resources enable extensions.claude-code-hook-adapter
```

Restart Varth after enabling the adapter. Discovered Claude Code hook commands also appear under the Hooks tab in `/resources`, where they can be enabled or disabled individually.

If those hooks depend on Claude Code skills, enable the separate skill compatibility extension:

```bash
varth resources enable extensions.claude-code-skills
```

That extension loads `~/.claude/skills` and the nearest project `.claude/skills` directory into Varth's native available-skills prompt, contributes them through Pi resource discovery so `/skill:name` works, and provides a Claude-compatible `Skill` tool for hooks that ask the model to invoke `Skill("name")`.
...
- `TaskCompleted` — fires at the end of each turn (pi `turn_end`); observer-only, block decisions are ignored. ⚠️ Note: this diverges from Claude Code, where `TaskCompleted` fires when a task-list item is marked completed (via the `TaskUpdate` tool) and can block; varth has no task-list tool, so this name is bound to per-turn semantics instead
- `TurnStart`, `MessageStart`, `MessageEnd`, `ModelSelect`, `UserBash` — varth-specific observer hooks for pi events with no Claude Code equivalent
- `SubagentStart` — fires when an `Agent` tool subagent spawns (varth `subagents:started` bus event); observer-only. Payload adds `subagent_id`, `subagent_type`, `description`, `visibility`
...
Plugin packages installed via `varth install` may ship a `hooks/hooks.json` (or `.claude-plugin/hooks/hooks.json`). Those hooks honor the same full event set as the adapter above; `SessionStart` context from packages is injected into the system prompt.
...
```bash
mkdir -p ~/.config/varth/harness/hooks/bash
cat > ~/.config/varth/harness/hooks/bash/git-status-short.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail

if [ "${VARTH_TOOL_INPUT_COMMAND:-}" = "git status" ]; then
  echo "git status --short --branch"
fi
SH
```

Start Varth and ask it to run `git status`. The displayed Bash command should show the rewritten command.

### Block `npm install`

```bash
mkdir -p ~/.config/varth/harness/hooks/bash
cat > ~/.config/varth/harness/hooks/bash/pnpm-only.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail

case "${VARTH_TOOL_INPUT_COMMAND:-}" in
  npm\ install*|npm\ i*)
...
```bash
mkdir -p .varth/hooks/bash
cat > .varth/hooks/bash/block-rm-root.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
```bash
varth resources enable hooks.bash.project.block-rm-root-sh
```

## RTK Hook

Varth's built-in RTK integration is exposed as:
...
- Restart is not required for hook enable/disable changes, but adding or removing hook files is easiest to verify by restarting Varth.
