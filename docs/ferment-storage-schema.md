...
1. **`VARTH_FERMENTS_DIR` env var** — explicit override. Used by terminal-bench-2 to land per-trial ferments under the bind-mounted `/logs/agent/ferments`, and by tests that need to pin storage to a tmpdir.
2. **`<project-root>/.varth/ferments`** — when the cwd is inside a git/package.json project. Keeps ferments co-located with the work.
3. **`~/.config/varth/ferments`** — global fallback when no project root is found.
