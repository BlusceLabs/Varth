...
Teleport is available when the `VARTH_EXPERIMENTAL_TELEPORT` environment variable is set:

```bash
VARTH_EXPERIMENTAL_TELEPORT=1 varth
```
...
- **An API key** — run `varth login` if you haven't already.
...
|---|---|
| `VARTH_REMOTE_ENDPOINT` | Override the remote API endpoint (advanced/internal use). |
| `VARTH_API_KEY` | Provide an API key without running `varth login`. |
...
When you exit the SSH session (`exit` or Ctrl-D), you return to varth.
...
When `--source` is a directory, default excludes (`node_modules/`, `dist/`, `.next/`, `target/`, `__pycache__/`, `.venv/`, `.env*`, `*.log`, `.DS_Store`, `.varth/`, …) are applied, plus your repo's gitignored entries unless `--include-ignored` is set. When `--source` is a single file, only explicit `--exclude` patterns are applied.
...
| `Ctrl+D` | Exit the overlay entirely and return to local varth |
...
When a session terminates normally (e.g. you type `exit` in the shell), its tab closes. If that was the last tab, the overlay exits and you return to local varth. A tab closed by a fatal error stays in the tab bar so you can see the banner and decide whether to retry or exit.

---

## Git credentials prompt

When you run `/teleport` and varth detects a git remote (e.g. `github.com`), it shows an interactive prompt asking for a personal access token. This token is forwarded to the sandbox so the remote agent can push and pull from private repositories.
...
If you previously saved a token for the detected host, varth uses it automatically. To skip the prompt entirely, use `/teleport --no-git-token`.

On subsequent `/terminal` calls or when reusing a workspace, varth automatically propagates saved credentials if they haven't been synced yet.
...
If you reference a session that no longer exists, varth will suggest using `/remote-sessions` to browse available sessions. Sessions that have finished and been cleaned up server-side cannot be reattached.
