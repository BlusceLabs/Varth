...
Ferment is Varth's multi-session project mode. Give it a goal, it breaks the work into phases and steps, executes them using subagent workers, grades the results, and resumes automatically across sessions until the project is complete.

## The core idea

> "The plan IS the state."

Every ferment is a single JSON file in `.varth/ferments/<uuid>.json`. The harness reads this file at session start, a deterministic engine decides what to do next, and every action — activating a phase, completing a step, recording a decision — updates the file immediately. If the process crashes or the session ends, the next session resumes from exactly where it left off.
...
before an agent run starts and stay fixed for that run. Varth therefore uses
...
| `worker` | Subagent worker context (`VARTH_SUBAGENT=1`) | Empty from ferment's perspective — workers get their toolset from the agents manager, including `submit_agent_report` for Ferment-linked workers |
...
- Varth asks what you want to ferment.
...
- If the request is ambiguous, the agent asks focused scoping questions with
  recommended answers.
- Varth shows the proposed markdown plan for review.
...
Sessions resume automatically. When you close and reopen Varth with an active ferment, the agent picks up exactly where it left off.
...
> /ferment pause
[you check something outside Varth]
...
Run Varth in headless mode with a pre-existing ferment:

```bash
VARTH_ACTIVE_FERMENT=<ferment-id> varth --headless
```
...
| Variable | Effect |
|----------|--------|
| `VARTH_ACTIVE_FERMENT` | ID of the ferment to resume on session start |
| `VARTH_SUBAGENT=1` | Set on worker child processes — skips session_start rehydration to prevent re-entrancy |

### Ferment state file

```
.varth/ferments/<uuid>.json
```
...
│ - Read/write `.varth/ferments/*.json` (atomic, cached)          │
...
| `src/ferment/store.ts` | Persistence — read/write `.varth/ferments/*.json` |
| `src/ferment/types.ts` | TypeScript types for all ferment data |
| `.varth/ferments/<uuid>.json` | Persisted ferment state (one file per ferment) |
