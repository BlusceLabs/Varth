...
import (
	...
	"github.com/castai/varth/tools/proxy-helper/pkg/cast"
...
func NewSSHProxyCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "SSH ProxyCommand bridge to a Varth remote sandbox",
		Long: `Acts as an SSH ProxyCommand bridge to a Varth remote sandbox.

Requires $VARTH_API_KEY to be set.
Override the API endpoint with $VARTH_REMOTE_ENDPOINT.

Example ~/.ssh/config entry:

  Host *.remote.varth.dev
    ProxyCommand varth-ssh-proxy ssh-proxy %h`,
