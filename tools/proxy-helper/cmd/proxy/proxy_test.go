...
// fakeBackend stands in for the Varth API + the sandbox WS endpoint.
...
func TestReadAPIKey(t *testing.T) {
	t.Setenv("VARTH_API_KEY", "env-key")
	...
	t.Setenv("VARTH_API_KEY", "")
	if _, err := cast.ReadAPIKey(); err == nil {
		t.Error("expected error when VARTH_API_KEY is unset")
	}
}
