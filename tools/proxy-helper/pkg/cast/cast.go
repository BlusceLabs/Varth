// Package cast provides API client helpers for talking to the Varth/CAST AI
...
func checkResponse(resp *http.Response, endpoint string) error {
	...
	case 401:
		return &RemoteAuthError{
			Msg:    fmt.Sprintf("Invalid API key - run 'varth setup' to authenticate: %s", endpoint),
			Status: 401,
		}
	case 403:
		return &RemoteAuthError{
...
const DefaultEndpoint = "https://app.varth.dev/api"

// ReadAPIKey returns the Varth API key from the environment.
func ReadAPIKey() (string, error) {
	if k := os.Getenv("VARTH_API_KEY"); k != "" {
		return k, nil
	}
	return "", errors.New("VARTH_API_KEY environment variable is not set")
}

// ResolveEndpoint returns the API endpoint, falling back to DefaultEndpoint.
func ResolveEndpoint() string {
	if e := os.Getenv("VARTH_REMOTE_ENDPOINT"); e != "" {
		return e
	}
