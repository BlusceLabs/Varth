...
describe("resource store", () => {
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "varth-resources-"))
	...
	it("uses VARTH_CODING_AGENT_DIR settings.json when set", () => {
		process.env.VARTH_CODING_AGENT_DIR = dir
	...
	it("persists resource overrides without clobbering unrelated settings", () => {
		const path = tempSettingsPath()
		writeJson(path, { theme: "varth-minimal", resources: { "tools.web_fetch": false } })
		...
		expect(JSON.parse(readFileSync(path, "utf-8")).theme).toBe("varth-minimal")
