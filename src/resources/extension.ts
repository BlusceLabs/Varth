...
export default function resourcesExtension(pi: ExtensionAPI): void {
	...
	pi.registerCommand("resources", {
		description: "View/change Varth hooks, tools, extensions, and plugins",
	...
	pi.registerCommand("hooks", {
		description: "View/change Varth hooks",
	...
	pi.registerCommand("plugins", {
		description: "View/change Varth plugins",
		handler: async (_args, ctx) => {
...
async function ensureRtkOnStartup(ctx: ExtensionContext): Promise<void> {
	...
	if (process.env.VARTH_RTK_AUTO_INSTALL === "0") return
