...
async function installRtkUnlocked(): Promise<RtkInstallResult> {
	...
	const tempDir = mkdtempSync(join(tmpdir(), "varth-rtk-"))
	try {
...
function rtkInstallDir(): string {
	return join(process.env.VARTH_CODING_AGENT_DIR ?? join(homedir(), ".config", "varth", "harness"), "rtk")
}

function globalBinDir(): string {
	if (process.platform === "win32") {
		return join(process.env.LOCALAPPDATA ?? join(homedir(), "AppData", "Local"), "Programs", "varth", "bin")
	}
