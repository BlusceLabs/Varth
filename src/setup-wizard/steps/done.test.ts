...
describe("runDoneStep", () => {
	...
	beforeEach(() => {
		...
		installRtkMock.mockResolvedValue({
			version: "v1.2.3",
			binaryPath: "/tmp/varth-test/rtk",
			linkPath: "/tmp/varth-test/bin/rtk",
		})
	...
	it("exports VARTH_API_KEY to the user's shell profile", async () => {
		const tool = getClaudeCodeTool()
		const writeSpy = vi.spyOn(tool, "write").mockResolvedValue()
		...
		expect(zshrc).toContain("export VARTH_API_KEY=test-key")
