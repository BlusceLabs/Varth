...
export async function promptTelemetry(_opts: { backable: boolean }): Promise<Outcome<boolean>> {
	note(
		"Varth collects usage data (commands run, models used, error rates) to improve the product. This data is associated with your account. No prompt content or code is collected.",
		"Usage telemetry",
	)
 ...
 *
 * The choice is persisted to ~/.config/varth/config.json's
 * telemetry.enabled. $VARTH_TELEMETRY_ENABLED still wins over the
