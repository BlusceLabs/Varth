...
/**
 * How tool subcommands talk to varth at runtime.
 *
 * - `override` (recommended): write varth settings into each tool's own
 *   config file (`~/.config/opencode/opencode.json`, `~/.claude/settings.json`,
 *   …). The tool keeps working standalone afterwards.
 * - `inject`: launch via `varth <tool>` only — env vars are set per-process,
