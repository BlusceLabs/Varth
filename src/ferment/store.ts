 ...
 * - Per-project storage: ferments live in .varth/ferments/ inside the repo, or global fallback
 */
...
export function getGlobalFermentsDir(): string {
	return resolve(homedir(), ".config", "varth", "ferments")
}
...
export function resolveFermentsDir(cwd?: string): string {
	...
	const envDir = process.env.VARTH_FERMENTS_DIR
	if (envDir) return envDir
	const project = detectProjectRoot(cwd)
	if (project) return resolve(project, ".varth", "ferments")
