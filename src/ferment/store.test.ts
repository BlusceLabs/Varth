...
describe("FermentStorage v4", () => {
	...
	describe("resolveFermentsDir env override", () => {
		const previous = process.env.VARTH_FERMENTS_DIR

		afterEach(() => {
			if (previous === undefined) {
				Reflect.deleteProperty(process.env, "VARTH_FERMENTS_DIR")
			} else {
				process.env.VARTH_FERMENTS_DIR = previous
			}
		})

		it("honors VARTH_FERMENTS_DIR when set", () => {
			const dir = createTempDir()
			process.env.VARTH_FERMENTS_DIR = dir
		...
		it("falls back to the project-root path when env is unset", () => {
			...
			const root = createTempDir()
			mkdirSync(join(root, ".git"))
			expect(resolveFermentsDir(root)).toBe(join(root, ".varth", "ferments"))
		})
	})
