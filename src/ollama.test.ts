...
describe("injectOllamaProvider — models.json merge", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "varth-ollama-test-"))
	...
	it("preserves varth-dev, varth-experimental, and custom providers", async () => {
		writeModelsJson({
			providers: {
				"varth-dev": { models: [{ id: "varth-1" }] },
				"varth-experimental": { models: [{ id: "exp-1" }] },
				"my-custom-provider": {
					api: "openai-completions",
		...
		expect(providers["varth-dev"].models).toEqual([{ id: "varth-1" }])
		expect(providers["varth-experimental"].models).toEqual([{ id: "exp-1" }])
	...
	it("is a no-op when probe returns no models", async () => {
		...
		expect(providers["varth-dev"]).toBeDefined()
	})
	...
	it("swallows thrown errors silently (does not propagate)", async () => {
		writeModelsJson({ providers: { "varth-dev": { models: [] } } })
...
describe("readOllamaModelsFromConfig + readOllamaModelMetadata", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "varth-ollama-read-"))
		modelsJsonPath = join(tmpDir, "models.json")
	})
	...
	it("returns [] when the file has no ollama provider", () => {
		writeFileSync(modelsJsonPath, JSON.stringify({ providers: { "varth-dev": { models: [] } } }), "utf-8")
		expect(readOllamaModelsFromConfig(modelsJsonPath)).toEqual([])
	})
...
describe("resolveOllamaHost — environment variable precedence", () => {
	...
	it("falls back to $VARTH_OLLAMA_HOST when $OLLAMA_HOST is unset", () => {
		process.env.VARTH_OLLAMA_HOST = "http://varth-host:11434"
		expect(resolveOllamaHost()).toBe("http://varth-host:11434")
	})

	it("$OLLAMA_HOST wins over $VARTH_OLLAMA_HOST", () => {
		process.env.OLLAMA_HOST = "http://primary:11434"
		process.env.VARTH_OLLAMA_HOST = "http://fallback:11434"
...
describe("readOllamaModelsFromConfig — defensive reads against malformed models.json", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "varth-ollama-defensive-"))
		modelsJsonPath = join(tmpDir, "models.json")
	})
	...
	it("returns [] when the file contains an unrelated provider but not ollama", () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				providers: {
					"varth-dev": { models: [{ id: "should-not-leak" }] },
				},
			}),
			"utf-8",
		)
...
describe("pi-mono ModelRegistry integration — persisted ollama block is accepted", () => {
	...
	beforeEach(() => {
		tmpDir = mkdtempSync(join(tmpdir(), "varth-ollama-registry-"))
		modelsJsonPath = join(tmpDir, "models.json")
	})
