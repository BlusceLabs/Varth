...
describe("isInSandboxCluster", () => {
	...
	it("returns true when VARTH_SANDBOX=1", () => {
		vi.stubEnv("VARTH_SANDBOX", "1")
		expect(isInSandboxCluster()).toBe(true)
	})

	it("returns true when VARTH_SANDBOX=true (case-insensitive)", () => {
		vi.stubEnv("VARTH_SANDBOX", "true")
		expect(isInSandboxCluster()).toBe(true)
	})

	it("returns false when VARTH_SANDBOX is not a truthy value", () => {
		vi.stubEnv("VARTH_SANDBOX", "0")
	...
	it("prioritises VARTH_SANDBOX over security-fallback signals", () => {
		vi.stubEnv("VARTH_SANDBOX", "1")
		expect(isInSandboxCluster()).toBe(true)
	})
