// Spawn a varth subprocess for one-shot agent work. Handles dev (tsx) vs bun-binary vs node-binary invocation.
...
export function getAgentInvocation(args: string[]): { command: string; args: string[] } {
	...
	if (process.argv[1].endsWith(".ts")) {
		...
		throw new Error("Dev mode requires tsx to spawn varth subprocesses, but node_modules/.bin/tsx was not found.")
	}
	return { command: process.execPath, args: [process.argv[1], ...args] }
}

export interface SpawnVarthOptions {
	args: string[]
...
export function spawnVarthSubprocess(opts: SpawnVarthOptions): ChildProcess {
