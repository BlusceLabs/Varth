 ...
 * setting VARTH_CLIPBOARD_FORCE=1. All other Linux systems require at least
 * one of DISPLAY / WAYLAND_DISPLAY to be set.
 */
export function hasDisplayServer(): boolean {
	if (process.platform !== "linux") return true
	if (process.env.VARTH_CLIPBOARD_FORCE === "1") return true
...
export function getNativeClipboard(): { clipboard: NativeClipboard | null; error: string | null } {
	...
	if (process.env.TERMUX_VERSION || !hasDisplayServer()) {
		loadError = process.env.TERMUX_VERSION
			...
			: "Clipboard image support is not available: no display server detected (WSL/headless Linux — set VARTH_CLIPBOARD_FORCE=1 to override)"
		cached = null
