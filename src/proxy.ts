 ...
 *   VARTH_PROXY      – explicit override, used for both HTTP and HTTPS
 *   HTTP_PROXY        – standard, http:// scheme
 *   HTTPS_PROXY       – standard, https:// scheme
 *   NO_PROXY          – comma/space separated list of hosts to bypass
 *   VARTH_NO_PROXY   – explicit override for the no-proxy list
 *
 * The upstream pi-coding-agent cli.js does the same thing, but varth
 * bypasses that entry point, so we replicate it here.
 */
export function installProxyAgent(): void {
	const httpProxy = process.env.VARTH_PROXY ?? process.env.HTTP_PROXY ?? process.env.http_proxy
	const httpsProxy = process.env.VARTH_PROXY ?? process.env.HTTPS_PROXY ?? process.env.https_proxy
	const noProxy = process.env.VARTH_NO_PROXY ?? process.env.NO_PROXY ?? process.env.no_proxy
