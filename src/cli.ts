...
import dumpExtension from "./extensions/dump.js"
...
import modelsCommandExtension from "./extensions/models-command.js"
...
import { writeVarthKeybindingDefaults } from "./extensions/permissions/keybindings.js"
...
import skillExtension from "./extensions/skill.js"
import snapshotExtension from "./extensions/snapshot.js"
...
import usageStatsExtension from "./extensions/usage-stats.js"
import varthMinimalTintsExtension from "./extensions/varth-minimal-tints.js"
...
try {
	// Top-level varth subcommands (setup, claude, opencode, …) and the
	...
	// the version using piConfig.name = "varth".
	...
	if (helpOrVersion) {
		...
		const envKey = process.env.VARTH_API_KEY || undefined
		...
		const agentDir = process.env.VARTH_CODING_AGENT_DIR
		if (!agentDir) {
			throw new Error("VARTH_CODING_AGENT_DIR is not set; cli.ts must be entered via entry.ts")
		}
		...
		// override in place.
		writeVarthKeybindingDefaults(agentDir)
		...
		try {
			...
			if ((err as NodeJS.ErrnoException).code === "ENOENT") {
				writeFileSync(settingsPath, `${JSON.stringify({ quietStartup: true, theme: "varth-minimal" }, null, 2)}\n`)
			} else {
		...
		// wanting custom colors should clone+rename (e.g. `my-varth.json`), which
		// this loop won't touch. The varth-minimal source keeps all six bg tokens
		// as `""` placeholders; the varth-minimal-tints extension fills them in
		...
		// the varth-minimal-tints and terminal-colors extensions. Skip non-TUI
		...
		// this loop injected per-process bg tints into varth-minimal.json on every
		// startup; that race is what made terminals clobber each other's colors.
		// Tints now live in memory only, applied by the varth-minimal-tints
		...
		// Clear the visible viewport and home the cursor so varth renders at the top.
		if (terminalStartupOutputAllowed) {
		...
		const fetchPatchedSymbol = Symbol.for("varth.fetchPatched")
		if (!(globalThis.fetch as typeof globalThis.fetch & { [key: symbol]: boolean })[fetchPatchedSymbol]) {
			const userAgent = `varth/${getVersion()}`
		...
		const terminalUiExtensionFactories = isTerminalUiMode(rawArgs, terminalIo)
			? [terminalColorsExtension, varthMinimalTintsExtension, uiExtension]
			: []
		...
		if (acpMode) {
			...
			// Delegate to pi-mono's CLI main function, injecting the varth extension
			const { main } = await import("@earendil-works/pi-coding-agent")
