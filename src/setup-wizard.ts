...
async function runMigrationPhase(d: MergedDiscovery): Promise<MigrationPhaseResult> {
	...
	const action = await clack.select<MigrationAction>({
		message: "Migrate MCP servers to Varth?",
		options: [
...
export async function runSetupWizard(options: {
	...
	clack.intro("Varth first-time setup")
	...
	if (merged?.hasAnythingMigratable) {
		...
		if (action === "migrate") {
			...
			if (serverCount > 0) {
				...
				clack.log.success(`Migrated ${serverCount} MCP server(s) to Varth.`)
			}
			...
			if (commandsCopied > 0) {
				clack.log.success(`Migrated ${commandsCopied} command(s) to Varth prompts.`)
			}
			migrationRan = true
	...
	if (options.needsSkillsSetup) {
		clack.note(
			"Varth will look for skill files in the selected directories.\n" +
