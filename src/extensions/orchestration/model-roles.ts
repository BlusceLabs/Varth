 ...
 *
 * Users can override in ~/.config/varth/harness/settings.json under the
 ...
 *     "reviewer": "varth-dev/minimax-m2.7",
...
export const DEFAULT_MODEL_ROLES: Readonly<ModelRoles> = {
	orchestrator: "varth-dev/kimi-k2.7",
	planner: "varth-dev/kimi-k2.7",
	builder: ["varth-dev/minimax-m3"],
	reviewer: ["varth-dev/kimi-k2.7"],
	explorer: "varth-dev/nemotron-3-ultra-fp4",
	researcher: "varth-dev/minimax-m3",
	judge: ["varth-dev/kimi-k2.7"],
}
		...
		if (key === "orchestrator") {
			if (typeof value !== "string" || value.trim().length === 0) {
				warnings.push({
					...
					message: `modelRoles.${key} must be a non-empty string (e.g. "varth-dev/kimi-k2.6"). Using default.`,
				})
				continue
			}
