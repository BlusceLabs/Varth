...
describe("loadModelMetadata", () => {
	const testDir = join(tmpdir(), `varth-model-metadata-test-${process.pid}`)
...
describe("resolveModelMetadata", () => {
	...
	it("returns builtin for known model (e.g. varth-dev/kimi-k2.6)", () => {
		const result = resolveModelMetadata("varth-dev/kimi-k2.6", testPath)
	...
	it("prefers custom over builtin when both exist", () => {
		...
		writeFileSync(
			...
			JSON.stringify(
				{
					modelMetadata: {
						"varth-dev/kimi-k2.6": { tier: "light", description: "Custom override" },
					},
				},
				null,
				2,
	...
	it("returns builtin for minimax-m2.7", () => {
		const result = resolveModelMetadata("varth-dev/minimax-m2.7", testPath)
		expect(result).toEqual(expect.objectContaining({ source: "builtin", tier: "standard", vision: false }))
	})

	it("returns builtin for minimax-m3", () => {
		const result = resolveModelMetadata("varth-dev/minimax-m3", testPath)
		expect(result).toEqual(expect.objectContaining({ source: "builtin", tier: "heavy", vision: true }))
	})

	it("returns builtin for nemotron-3-ultra-fp4", () => {
		const result = resolveModelMetadata("varth-dev/nemotron-3-ultra-fp4", testPath)
...
describe("isModelMetadataMissing", () => {
	...
	it("returns false for known builtin model", () => {
		expect(isModelMetadataMissing("varth-dev/kimi-k2.6", testPath)).toBe(false)
		expect(isModelMetadataMissing("varth-dev/minimax-m2.7", testPath)).toBe(false)
		expect(isModelMetadataMissing("varth-dev/minimax-m3", testPath)).toBe(false)
	})
