...
describe("rewriteWithRtk", () => {
	...
	let previousVarthDir: string | undefined
	let previousHome: string | undefined

	beforeEach(() => {
		_resetRtkState()
		previousVarthDir = process.env.VARTH_CODING_AGENT_DIR
		previousHome = process.env.HOME
		tmpRtkRoot = mkdtempSync(join(tmpdir(), "varth-rtk-rewrite-test-"))
		process.env.VARTH_CODING_AGENT_DIR = tmpRtkRoot
	...
	afterEach(() => {
		...
		if (previousVarthDir === undefined) delete process.env.VARTH_CODING_AGENT_DIR
		else process.env.VARTH_CODING_AGENT_DIR = previousVarthDir
		// biome-ignore lint/performance/noDelete: env-var cleanup needs a real delete.
...
describe("rtkSpawnHook", () => {
	...
	beforeEach(() => {
		...
		previousVarthDir = process.env.VARTH_CODING_AGENT_DIR
		previousHome = process.env.HOME
		tmpRtkRoot = mkdtempSync(join(tmpdir(), "varth-rtk-spawn-test-"))
		process.env.VARTH_CODING_AGENT_DIR = tmpRtkRoot
		process.env.HOME = tmpRtkRoot
		mockExecFileSync.mockReset()
		mockExecFile.mockReset()
