 ...
 *
 * Uses pi's official loadSkillsFromDir API to discover skills from varth's DEFAULT_SKILL_PATHS.
 */
 ...
 * in varth's DEFAULT_SKILL_PATHS. Project-level paths are resolved relative to cwd;
...
export function preloadSkills(skillNames: string[], cwd: string): PreloadedSkill[] {
	...
	for (const name of skillNames) {
		...
		if (!skill) {
			results.push({ name, content: `(Skill "${name}" not found in varth skill paths)` })
			continue
		}
