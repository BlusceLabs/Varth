...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		// .varth/agents/ directory and forgets the previous one.
	...
	const buildTypeListText = () => {
		...
		return [
			...
			"",
			`Custom agents can be defined in .varth/agents/<name>.md (project) or ${getAgentDir()}/agents/<name>.md (global) - they are picked up automatically. Project-level agents override global ones. Creating a .md file with the same name as a default agent overrides it.`,
...
${AGENT_TOOL_GUIDELINES}`,
			parameters: Type.Object({
				...
				subagent_type: Type.String({
					description: `The type of specialized agent to use. Available types: ${getAvailableTypes().join(", ")}. Custom agents from .varth/agents/*.md (project) or ${getAgentDir()}/agents/*.md (global) are also available.`,
				}),
				model: Type.Optional(
					Type.String({
			...
			renderResult(result, { expanded: piExpanded, isPartial }, theme, context) {
				// Wire varth's global ctrl+o cycle: register on first render, then read the
			...
			execute: async (toolCallId, params, signal, onUpdate, ctx) => {
				...
				// LLMs and personas cannot create hidden agents. Internal varth callers (e.g. permission
	...
	const projectAgentsDir = () => join(process.cwd(), ".varth", "agents")
	...
	async function showGenerateWizard(ctx: ExtensionCommandContext, targetDir: string) {
		...
		const generatePrompt = `Create a custom varth sub-agent definition file based on this description: "${description}"
...
models: <optional ordered list of models, e.g. ["varth-dev/minimax-m2.7"]. Omit to inherit parent model>
...
Write the file using the write tool. Only write the file, nothing else.`
	...
	async function showManualWizard(ctx: ExtensionCommandContext, targetDir: string) {
		...
		if (wizardModelChoices.includes(modelChoice)) {
			modelLine = `\nmodels: ["${VARTH_DEV_PROVIDER}/${modelChoice}"]`
