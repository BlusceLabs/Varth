...
describe("getInstalledPackageResourceDirs", () => {
	...
	it("returns both paths when two packages both have the subdir", () => {
		...
		vi.mocked(getConfiguredPackageResourceRecords).mockReturnValueOnce([
			{ id: "plugins.package.p1", installedPath: pkg1Dir, source: "p1", scope: "user", origin: "varth" },
			{ id: "plugins.package.p2", installedPath: pkg2Dir, source: "p2", scope: "user", origin: "varth" },
		])
	...
	it("silently skips packages with no installedPath", () => {
		...
		vi.mocked(getConfiguredPackageResourceRecords).mockReturnValueOnce([
			{ id: "plugins.package.p1", installedPath: undefined, source: "p1", scope: "user", origin: "varth" },
			{ id: "plugins.package.p2", installedPath: pkgWithPath, source: "p2", scope: "user", origin: "varth" },
		])
	...
	it("honors the subdir parameter — agents vs skills return different paths", () => {
		...
		vi.mocked(getConfiguredPackageResourceRecords)
			.mockReturnValueOnce([
				{ id: "plugins.package.p", installedPath: pkgDir, source: "p", scope: "user", origin: "varth" },
			])
			.mockReturnValueOnce([
				{ id: "plugins.package.p", installedPath: pkgDir, source: "p", scope: "user", origin: "varth" },
			])
