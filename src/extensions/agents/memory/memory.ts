 ...
 *
 * Memory scopes:
 *   - "user"    → ~/.config/varth/harness/agent-memory/{agent-name}/
 *   - "project" → .varth/agent-memory/{agent-name}/
 *   - "local"   → .varth/agent-memory-local/{agent-name}/
 */
...
export function resolveMemoryDir(agentName: string, scope: MemoryScope, cwd: string): string {
	...
	switch (scope) {
		...
		case "project":
			return join(cwd, ".varth", "agent-memory", agentName)
		case "local":
			return join(cwd, ".varth", "agent-memory-local", agentName)
	}
}
