...
/** Cached varth config — loaded once per process to avoid repeated disk reads per agent spawn. */
const varthConfig = loadConfig()
...
async function runAgentInner(
	...
	if (debugSession) {
		try {
			const debugDir = join(effectiveCwd, ".varth", "debug", debugSession)
	...
	// Repo-native extensions registered directly by the Varth CLI are not
	...
	settingsManager.applyOverrides({ retry: { maxRetries: varthConfig.retry.maxRetries } })
	...
	// apply persona-specific path scopes (e.g. plan persona → .varth/plans/).
	const prevPersona = process.env.VARTH_AGENT_PERSONA
	if (agentConfig?.name) {
		process.env.VARTH_AGENT_PERSONA = agentConfig.name
	}
	...
	try {
		...
		if (agentConfig?.name) {
			...
			if (prevPersona === undefined) {
				...
				delete process.env.VARTH_AGENT_PERSONA
			} else {
				process.env.VARTH_AGENT_PERSONA = prevPersona
			}
		}
		if (personaPhase) {
