...
// - Global:  ~/.config/varth/harness/agents.json (via getAgentDir()) — manual defaults, never written here
// - Project: <cwd>/.varth/agents.json — written by /agents → Settings; overrides global on load
...
function projectPath(cwd: string): string {
	return join(cwd, ".varth", "agents.json")
}
...
function readSettingsFile(path: string): SubagentsSettings {
	...
	try {
		...
		console.warn(`[varth-agents] Ignoring malformed settings at ${path}: ${reason}`)
		return {}
	}
}
