...
const FAKE_AGENT_DIR = join(tmpdir(), `varth-global-${Date.now()}`)
...
describe("discovery-priority: project agents override global", () => {
	...
	it("project .varth/agents/ overrides global agent with same name", () => {
		const cwd = join(tmpdir(), `varth-project-${Date.now()}`)
		...
		const projectAgentsDir = join(cwd, ".varth", "agents")
	...
	it("global agent is returned when no project override exists", () => {
		const cwd = join(tmpdir(), `varth-global-only-${Date.now()}`)
	...
	it("project agent dirs use .varth not .pi", () => {
		const cwd = join(tmpdir(), `varth-path-check-${Date.now()}`)
		const projectAgentsDir = join(cwd, ".varth", "agents")
		writeAgentMd(projectAgentsDir, "path-check-agent", "varth path agent")
