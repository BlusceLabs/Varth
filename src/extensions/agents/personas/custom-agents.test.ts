...
const FAKE_AGENT_DIR = join(tmpdir(), `varth-global-custom-agents-${Date.now()}`)
...
describe("AgentConfig.tokenBudget parsing", () => {
	...
	beforeEach(() => {
		...
		projectDir = join(tmpdir(), `varth-project-token-budget-${Date.now()}`)
		projectAgentsDir = join(projectDir, ".varth", "agents")
	})
...
describe("custom agents — user override hierarchy preserves new fields", () => {
	...
	beforeEach(() => {
		tmpRoot = join(tmpdir(), `varth-hierarchy-${Date.now()}`)
