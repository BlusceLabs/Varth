...
const FAKE_AGENT_DIR = join(homedir(), ".config", "varth", "harness")
...
// env var is PI_CODING_AGENT_DIR. In production (the varth binary), varth's
// package.json sets piConfig.name=varth, so the var becomes VARTH_CODING_AGENT_DIR.
describe("resolveMemoryDir", () => {
	...
	it("project scope resolves under <cwd>/.varth/agent-memory/", () => {
		const cwd = "/some/project"
		const dir = resolveMemoryDir("my-agent", "project", cwd)
		expect(dir).toBe(join(cwd, ".varth", "agent-memory", "my-agent"))
	})

	it("local scope resolves under <cwd>/.varth/agent-memory-local/", () => {
		const cwd = "/some/project"
		const dir = resolveMemoryDir("my-agent", "local", cwd)
		expect(dir).toBe(join(cwd, ".varth", "agent-memory-local", "my-agent"))
	})
