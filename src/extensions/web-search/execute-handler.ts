...
export const SEARCH_ENDPOINT = "https://llm.varth.dev/v1/search"
...
export async function executeWebSearch(params: WebSearchParams, signal?: AbortSignal): Promise<WebSearchResult> {
	...
	if (!apiKey) {
		throw new Error(
			"Web search requires an API key. Run 'varth' and log in, or visit https://app.varth.dev to create a key.",
		)
	}
