...
export async function suggestSessionName(ctx: ExtensionContext, hint?: string, quiet = false): Promise<string> {
	...
	if (!resolvedHint) {
		if (!quiet) {
			if (ctx.hasUI) {
				...
				console.error("[varth] auto-naming failed: no user message found")
			}
		}
	...
	const apiKey = config.apiKey || process.env.VARTH_API_KEY || ""
	...
	if (!apiKey) {
		if (!quiet) {
			if (ctx.hasUI) {
				...
				console.error("[varth] auto-naming failed: no API key")
			}
		}
		return deterministicFallback(base)
	}
	...
	try {
		...
		if (!response.ok) {
			...
			if (!quiet) {
				if (ctx.hasUI) {
					...
					console.error(`[varth] auto-naming API error: ${response.status} ${response.statusText} ${errorBody}`)
				}
			}
			return deterministicFallback(base)
		}
		...
		if (!quiet) {
			if (ctx.hasUI) {
				...
				console.error("[varth] auto-naming: LLM returned empty suggestion")
			}
		}
		return deterministicFallback(base)
	} catch (err) {
		if (!quiet) {
			if (ctx.hasUI) {
				...
				console.error(`[varth] auto-naming exception: ${err}`)
			}
		}
		return deterministicFallback(base)
	}
