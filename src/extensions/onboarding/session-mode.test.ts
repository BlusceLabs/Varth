...
describe("buildSessionModeLaunchContext", () => {
	...
	it("uses Varth's non-interactive pre-dispatch classification", () => {
...
describe("session-mode onboarding persistence", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "varth-session-mode-onboarding-"))
	...
	it("preserves picker selection when pi-tui re-invokes the widget factory", async () => {
		...
		harness.ui.setWidget("varth-session-mode-onboarding", lastFactory, calls[calls.length - 1][2])
