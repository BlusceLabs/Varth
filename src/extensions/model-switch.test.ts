...
const MODELS: ModelEntry[] = [
	{ id: "kimi-k2.6", provider: "varth-dev", name: "Kimi K2.6", input: ["text", "image"], contextWindow: 200_000 },
	{ id: "minimax-m2.7", provider: "varth-dev", name: "MiniMax M2.7", input: ["text"], contextWindow: 100_000 },
	{
...
function makeMockCtx() {
	return {
		model: { id: "kimi-k2.6", provider: "varth-dev", input: ["text", "image"] },
...
describe("modelSwitchExtension", () => {
	...
	describe("input validation", () => {
		const invalidInputs: Array<{ label: string; value: string }> = [
			...
			{ label: "trailing slash (missing model)", value: "varth-dev/" },
			{ label: "extra slash (three parts)", value: "varth-dev/kimi/k2.6" },
		]
		...
		for (const { label, value } of invalidInputs) {
			it(`rejects "${label}" without calling setModel`, async () => {
				...
				expect(textOf(result)).toContain("varth-dev/kimi-k2.6")
				expect(textOf(result)).toContain("varth-dev/minimax-m2.7")
		...
		it("sorts available models alphabetically in invalid-format error message", async () => {
			...
			const idxKimi = text.indexOf("varth-dev/kimi-k2.6")
			const idxMinimax = text.indexOf("varth-dev/minimax-m2.7")
	...
	describe("model lookup", () => {
		it("returns 'Model not found' when registry has no matching entry", async () => {
			const h = createHarness()
			const result = await h.exec("varth-dev/does-not-exist")

			expect(h.find).toHaveBeenCalledWith("varth-dev", "does-not-exist")
			expect(textOf(result)).toContain("Model not found: varth-dev/does-not-exist")
			expect(textOf(result)).toContain("Available models:")
			expect(textOf(result)).toContain("varth-dev/kimi-k2.6")
			expect(h.setModel).not.toHaveBeenCalled()
			expect(result.details).toBeNull()
		...
		it("handles missing modelRegistry on lookup (empty available list)", async () => {
			const h = createHarness()
			const result = await h.exec("varth-dev/kimi-k2.6", { omitRegistry: true })

			expect(textOf(result)).toContain("Model not found: varth-dev/kimi-k2.6")
			expect(h.setModel).not.toHaveBeenCalled()
		})
	})
	...
	describe("successful switch", () => {
		it("calls pi.setModel with the resolved descriptor and reports success", async () => {
			const h = createHarness()
			const result = await h.exec("varth-dev/kimi-k2.6")

			expect(h.find).toHaveBeenCalledWith("varth-dev", "kimi-k2.6")
			...
			expect(textOf(result)).toBe("Switched to model varth-dev/kimi-k2.6 (Kimi K2.6)")
			expect(result.details).toBeNull()
		})
	...
	describe("vision compatibility guard", () => {
		...
		it("rejects switch to non-vision model when session has images", async () => {
			...
			const result = await h.exec("varth-dev/minimax-m2.7")
			expect(h.setModel).not.toHaveBeenCalled()
			expect(textOf(result)).toContain("Current conversation contains images")
			expect(textOf(result)).toContain('target model "varth-dev/minimax-m2.7" does not support vision input')
			expect(result.details).toBeNull()
		})
		...
		it("allows switch to vision-capable model when session has images", async () => {
			...
			expect(textOf(result)).toContain("Switched to model varth-dev/kimi-k2.6 (Kimi K2.6)")
		})
		...
		it("allows switch to non-vision model when session has no images", async () => {
			...
			const result = await h.exec("varth-dev/minimax-m2.7")
			expect(h.setModel).toHaveBeenCalled()
			expect(textOf(result)).toContain("Switched to model varth-dev/minimax-m2.7 (MiniMax M2.7)")
		})
		...
		it("allows switch between non-vision models when session has images", async () => {
			...
			const result = await h.exec("varth-dev/nemotron-3-ultra-fp4", {
				currentModel: { id: "minimax-m2.7", provider: "varth-dev", name: "MiniMax M2.7", input: ["text"] },
			})
			expect(h.setModel).toHaveBeenCalled()
			expect(textOf(result)).toContain("Switched to model varth-dev/nemotron-3-ultra-fp4")
		})
	})
	...
	describe("MODEL_CAPABILITIES lookup", () => {
		...
		it("getModelTier returns correct tier for known models via the tool execution context", async () => {
			const { MODEL_CAPABILITIES } = await import("./orchestration/model-registry/builtin-models.js")
			const fakeModel = { id: "kimi-k2.6", provider: "varth-dev", input: ["text", "image"] } as { id: string }
		...
		it("getModelTier returns heavy for kimi-k2.6 and light for nemotron (fresh import)", async () => {
			const { MODEL_CAPABILITIES } = await import("./orchestration/model-registry/builtin-models.js")
			const currentTier = getModelTier({ id: "kimi-k2.6", provider: "varth-dev" } as never, MODEL_CAPABILITIES)
			const targetTier = getModelTier(
				{ id: "nemotron-3-ultra-fp4", provider: "varth-dev" } as never,
				MODEL_CAPABILITIES,
			)
	...
	describe("set_model tool result", () => {
		...
		it("does not include tier warning in tool result", async () => {
			const h = createHarness()
			const result = await h.exec("varth-dev/nemotron-3-ultra-fp4", {
				currentModel: { id: "kimi-k2.6", provider: "varth-dev", name: "Kimi K2.6" },
			})
			expect(h.setModel).toHaveBeenCalled()
		...
		it("does NOT append a warning when current model is not in MODEL_CAPABILITIES", async () => {
			const h = createHarness()
			const result = await h.exec("varth-dev/nemotron-3-ultra-fp4", {
				currentModel: { id: "unknown-model", provider: "varth-dev", name: "Unknown Model" },
			})
			expect(h.setModel).toHaveBeenCalled()
			const text = textOf(result)
	...
	describe("metadata wizard removed", () => {
		...
		it("does not call ctx.ui.select or ctx.ui.input on a known model", async () => {
			...
			await tool.execute("id", { model: "varth-dev/kimi-k2.6" }, undefined, undefined, ctx)
	...
	describe("__resetModelSwitchStateForTest", () => {
		...
		it("restores the model_select guard so it runs after being suppressed by set_model", async () => {
			...
			await trigger(
				...
				{
					type: "model_select",
					model: { id: "minimax-m2.7", provider: "varth-dev", input: ["text"], contextWindow: 100_000 },
					previousModel: { id: "kimi-k2.6", provider: "varth-dev", input: ["text", "image"] },
					source: "set",
				},
				{
					model: { id: "kimi-k2.6", provider: "varth-dev", input: ["text", "image"] },
	...
	describe("setModel failure", () => {
		it("returns a 'no API key' style message when pi.setModel resolves false", async () => {
			...
			expect(h.setModel).toHaveBeenCalledTimes(1)
			expect(textOf(result)).toContain("Failed to switch to varth-dev/kimi-k2.6")
		...
		describe("set_model tool rejects when getContextUsage returns null but local estimate is too large", () => {
			it("rejects switch to kimi-k2.6 when large context accumulated (null upstream tokens)", async () => {
				...
				h.find.mockImplementation((provider: string, id: string) => {
					...
					if (found && found.id === "kimi-k2.6" && found.provider === "varth-dev") {
	...
	describe("model_select handler", () => {
		...
		it("skips when no previousModel", async () => {
			...
			await trigger(
				...
				{
					type: "model_select",
					model: { id: "kimi-k2.6", provider: "varth-dev", input: ["text", "image"], contextWindow: 200_000 },
		...
		it("allows when tokens fit within target context window", async () => {
			...
			await trigger(
				"model_select",
				{
					type: "model_select",
					model: { id: "kimi-k2.6", provider: "varth-dev", input: ["text", "image"], contextWindow: 200_000 },
					previousModel: { id: "minimax-m2.7", provider: "varth-dev", input: ["text"] },
					source: "set",
				},
				mockCtx({ tokens: 10_000 }),
			)
		...
		it("allows switch between non-vision models when session has images", async () => {
			...
			await trigger(
				...
				{
					type: "model_select",
					model: { id: "nemotron-3-ultra-fp4", provider: "varth-dev", input: ["text"], contextWindow: 1_000_000 },
					previousModel: { id: "minimax-m2.7", provider: "varth-dev", input: ["text"] },
					source: "set",
				},
				mockCtx({ tokens: 10_000, ui: { notify } }),
		...
		it("syncs multi-model process flag to extension state on model_select from /models UI", async () => {
			...
			;(process as NodeJS.Process & { __varthMultiModelEnabled?: boolean }).__varthMultiModelEnabled = false
		...
		it("reverts when getContextUsage returns null but local estimate exceeds target context window", async () => {
			...
			await h.trigger(
				...
				{
					type: "model_select",
					model: { id: "kimi-k2.6", provider: "varth-dev", input: ["text", "image"], contextWindow: 10_000 },
					previousModel: { id: "claude-opus-4.6", provider: "varth-dev", input: ["text", "image"] },
					source: "set",
				},
		...
		it("allows switch when getContextUsage returns null but local estimate fits within target context", async () => {
			...
			await h.trigger(
				...
				{
					type: "model_select",
					model: { id: "kimi-k2.6", provider: "varth-dev", input: ["text", "image"], contextWindow: 262_144 },
					previousModel: { id: "claude-opus-4.6", provider: "varth-dev", input: ["text", "image"] },
					source: "set",
				},
				mockCtx({ getContextUsage: () => ({ tokens: null as unknown as number }) }),
