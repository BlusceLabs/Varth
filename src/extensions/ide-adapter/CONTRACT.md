...
This document defines the contract between the Varth harness and external IDE plugins (IntelliJ IDEA, VS Code, Neovim, etc.). Any IDE plugin that follows this contract will be auto-discovered and connected by the Varth harness.

## Overview

1. The IDE plugin starts a **WebSocket server** bound to `127.0.0.1` on a random free port.
2. The plugin writes a **lockfile** to `~/.config/varth/ide/<port>.lock`.
3. The Varth harness scans this directory, finds a matching lockfile, and connects via WebSocket.
4. The plugin exposes tools over the **Model Context Protocol (MCP)** and sends **notifications** to the harness.

## Lockfile Format

Path: `$XDG_CONFIG_HOME/varth/ide/<port>.lock` (`~/.config/varth/ide/` by default)
...
Sent when the user selects code and clicks "Send to Varth".
...
|---|---|
| `VARTH_IDE_LOCKFILE_DIR` | Override the default `~/.config/varth/ide/` directory. Useful for testing. |
