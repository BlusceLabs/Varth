...
Generic adapter that discovers IDE plugins via lockfiles and exposes their tools to Varth over the Model Context Protocol (MCP).
...
1. **Discovery**: Scans `$XDG_CONFIG_HOME/varth/ide/` for `*.lock` files written by IDE plugins.
