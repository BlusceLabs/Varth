...
describe("lockfile", () => {
	describe("getLockfileDir", () => {
		it("returns the default dot-config path when env is not set", () => {
			const original = process.env.VARTH_IDE_LOCKFILE_DIR
			Reflect.deleteProperty(process.env, "VARTH_IDE_LOCKFILE_DIR")
			const dir = getLockfileDir()
			expect(dir).toMatch(/\.config[/\\]varth[/\\]ide/)
			if (original !== undefined) {
				process.env.VARTH_IDE_LOCKFILE_DIR = original
			}
		})

		it("respects VARTH_IDE_LOCKFILE_DIR override", () => {
			const original = process.env.VARTH_IDE_LOCKFILE_DIR
			process.env.VARTH_IDE_LOCKFILE_DIR = "/tmp/test-ide"
			const dir = getLockfileDir()
