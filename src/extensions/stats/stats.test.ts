...
describe("getProviderDisplayName", () => {
	...
	it("maps pi-otel to Varth", () => {
		expect(getProviderDisplayName("pi-otel")).toBe("Varth")
	})
...
describe("getSourceName", () => {
	...
	it("maps pi-otel to Varth", () => {
		expect(getSourceName("pi-otel")).toBe("Varth")
	})
...
describe("sortFn", () => {
	const fixtures = [
		...
		{ modelName: "gpt-4", source: "Varth", cost: 8, inputTokens: 800, outputTokens: 400 },
	]
	...
	describe("sort by cost", () => {
		...
		it("uses modelName then source as tiebreaker", () => {
			const tied = [
				...
				{ modelName: "gpt-4", source: "Varth", cost: 10, inputTokens: 200, outputTokens: 200 },
			...
			expect(sorted[1].source).toBe("Varth")
	...
	describe("sort by tokens", () => {
		...
		it("uses modelName then source as tiebreaker", () => {
			const tied = [
				...
				{ modelName: "gpt-4", source: "Varth", cost: 2, inputTokens: 1000, outputTokens: 1000 },
			]
			const sorted = [...tied].sort((a, b) => sortFn(a, b, "tokens"))
			expect(sorted[0].source).toBe("Varth")
	...
	describe("sort by model", () => {
		...
		it("uses source as tiebreaker for same model", () => {
			...
			expect(gpt4Entries[0].source).toBe("Varth")
...
describe("formatAnalyticsVisual", () => {
	...
	it("coerces string int64 token values from protobuf JSON encoding", () => {
		...
		expect(aggregated.perModel.get("gpt-4\tVarth")?.inputTokens).toBe(1000)
		expect(aggregated.perModel.get("gpt-4\tVarth")?.outputTokens).toBe(500)
		expect(aggregated.perModel.get("claude-3\tVarth")?.inputTokens).toBe(2000)
		expect(aggregated.perModel.get("claude-3\tVarth")?.outputTokens).toBe(1000)
	})
})
...
describe("formatAnalyticsSummary", () => {
	...
	it("coerces string int64 token values from protobuf JSON encoding", () => {
		...
		expect(aggregated.totals.totalInput).toBe(3000)
		expect(aggregated.totals.totalOutput).toBe(1500)
		expect(aggregated.perModel.get("gpt-4\tVarth")?.inputTokens).toBe(3000)
		expect(aggregated.perModel.get("gpt-4\tVarth")?.outputTokens).toBe(1500)
	})
})
