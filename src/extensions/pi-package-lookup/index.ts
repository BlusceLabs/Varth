...
const ORIGINAL_PI_MANAGER = Symbol.for("varth.originalPiPackageManager")

type PackageScope = "user" | "project"
type SourceScope = PackageScope | "temporary"
type PackageOrigin = "varth" | "pi"
...
export function getOriginalPiAgentDir(): string {
	const explicitAgentDir = process.env.VARTH_ORIGINAL_PI_CODING_AGENT_DIR
	if (explicitAgentDir) return resolveTilde(explicitAgentDir)

	const piAgentDir = process.env.PI_CODING_AGENT_DIR
	if (piAgentDir && !isVarthAgentDir(piAgentDir)) return resolveTilde(piAgentDir)

	return resolveTilde(join(homedir(), ORIGINAL_PI_CONFIG_DIR_NAME, "agent"))
}

export function isOriginalPiPackageLookupEnabled(settingsPath = varthSettingsPath()): boolean {
	try {
...
export function getOriginalPiConfiguredPackages(cwd = process.cwd()): ConfiguredPackageEntry[] {
	if (!isOriginalPiPackageLookupEnabled() || isOriginalPiAgentDirSameAsVarth()) return []
...
export async function resolveOriginalPiPackageResources(
	...
	if (!isOriginalPiPackageLookupEnabled() || isOriginalPiAgentDirSameAsVarth()) return emptyResolvedPaths()
...
function isOriginalPiAgentDirSameAsVarth(): boolean {
	try {
		return resolve(getOriginalPiAgentDir()) === resolve(getVarthAgentDir())
	} catch {
		return false
	}
}

function getVarthAgentDir(): string {
	return process.env.VARTH_CODING_AGENT_DIR
		? resolve(process.env.VARTH_CODING_AGENT_DIR)
		: join(homedir(), ".config", "varth", "harness")
}

function isVarthAgentDir(path: string): boolean {
	try {
		return resolveTilde(path) === getVarthAgentDir()
	} catch {
		return false
	}
}

function varthSettingsPath(): string {
	return join(getVarthAgentDir(), "settings.json")
}
