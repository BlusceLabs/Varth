...
describe("original pi package lookup", () => {
	let dir: string
	let oldVarthAgentDir: string | undefined
	let oldPiAgentDir: string | undefined
	let oldOriginalPiAgentDir: string | undefined
	let oldHome: string | undefined

	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "varth-pi-package-lookup-"))
		oldVarthAgentDir = process.env.VARTH_CODING_AGENT_DIR
		oldPiAgentDir = process.env.PI_CODING_AGENT_DIR
		oldOriginalPiAgentDir = process.env.VARTH_ORIGINAL_PI_CODING_AGENT_DIR
		oldHome = process.env.HOME
	})

	afterEach(() => {
		restoreEnv("VARTH_CODING_AGENT_DIR", oldVarthAgentDir)
		restoreEnv("PI_CODING_AGENT_DIR", oldPiAgentDir)
		restoreEnv("VARTH_ORIGINAL_PI_CODING_AGENT_DIR", oldOriginalPiAgentDir)
		restoreEnv("HOME", oldHome)
		rmSync(dir, { recursive: true, force: true })
	})

	it("defaults pi package lookup off and honors the Varth resource override", () => {
		const agentDir = join(dir, "varth-agent")
		process.env.VARTH_CODING_AGENT_DIR = agentDir
	...
	it("falls back to the default pi agent dir when PI_CODING_AGENT_DIR is Varth's shim", () => {
		const homeDir = join(dir, "home")
		const varthAgentDir = join(dir, "varth-agent")
		process.env.HOME = homeDir
		process.env.VARTH_CODING_AGENT_DIR = varthAgentDir
		process.env.PI_CODING_AGENT_DIR = varthAgentDir

		expect(getOriginalPiAgentDir()).toBe(join(homeDir, ".pi", "agent"))
	})

	it("discovers original pi packages when PI_CODING_AGENT_DIR is Varth's shim", () => {
		const homeDir = join(dir, "home")
		const varthAgentDir = join(dir, "varth-agent")
		...
		process.env.VARTH_CODING_AGENT_DIR = varthAgentDir
		process.env.PI_CODING_AGENT_DIR = varthAgentDir
		mkdirSync(varthAgentDir, { recursive: true })
		mkdirSync(piAgentDir, { recursive: true })
		writeFileSync(
			join(varthAgentDir, "settings.json"),
			JSON.stringify({ resources: { [PI_PACKAGE_LOOKUP_RESOURCE_ID]: true } }),
		)
	...
	it("merges original pi resources behind native Varth resources without duplicating paths", () => {
		const primary = resolvedPaths({
			extensions: [
				{
					path: "/varth/pkg/extensions/index.js",
					enabled: true,
