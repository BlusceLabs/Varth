...
function ws(over: Partial<Workspace> = {}): Workspace {
	return {
		...
		status: "active",
		host: "trusting-ruling-frontier-486e4e-abbf.remote.varth.dev",
		...over,
	}
}
...
describe("leftmostLabel", () => {
	it("returns segment before first dot", () => {
		...
		expect(leftmostLabel("trusting-ruling-frontier-abbf.remote.varth.dev")).toBe("trusting-ruling-frontier-abbf")
	})
...
describe("matchesHostNickname", () => {
	const HOST = "trusting-ruling-frontier-486e4e-abbf.remote.varth.dev"
...
describe("resolveWorkspaceRef", () => {
	it("returns {id, name} when a UUID ref matches a known workspace", async () => {
		listWorkspacesMock.mockResolvedValue([ws({ id: UUID_A, name: "varth-dev" })])
		const { ctx } = makeCtx()
		const resolved = await resolveWorkspaceRef(ctx, UUID_A, { onEmpty: { kind: "mint" } })
		expect(resolved).toEqual({ id: UUID_A, name: "varth-dev" })
	...
	it("resolves a unique name match (case-insensitive) with the workspace's stored name", async () => {
		listWorkspacesMock.mockResolvedValue([ws({ id: UUID_A, name: "varth-dev" })])
		const { ctx } = makeCtx()
		const resolved = await resolveWorkspaceRef(ctx, "VARTH-DEV", { onEmpty: { kind: "mint" } })
		expect(resolved).toEqual({ id: UUID_A, name: "varth-dev" })
	})
	...
	it("refuses with disambiguation when ref hits two different workspaces", async () => {
		listWorkspacesMock.mockResolvedValue([
			ws({ id: UUID_A, name: "varth-dev", host: "alpha-foo.remote.varth.dev" }),
			ws({ id: UUID_B, name: "other", host: "varth-dev-x.remote.varth.dev" }),
		])
		const { ctx, ui } = makeCtx()
		await expect(resolveWorkspaceRef(ctx, "varth-dev", { onEmpty: { kind: "mint" } })).rejects.toBeInstanceOf(
			TeleportRefusal,
		)
	...
	it("refuses with disambiguation when two workspaces share the same host-nickname prefix", async () => {
		listWorkspacesMock.mockResolvedValue([
			ws({ id: UUID_A, name: "a", host: "trusting-ruling-frontier-x.remote.varth.dev" }),
			ws({ id: UUID_B, name: "b", host: "trusting-ruling-otter-y.remote.varth.dev" }),
		])
		const { ctx, ui } = makeCtx()
