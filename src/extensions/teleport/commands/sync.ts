...
export async function runSync(rawArgs: string, ctx: TeleportContext): Promise<void> {
	...
	if (!ctx.apiKey) {
		refuse(ctx, "No API key configured. Run `varth login`.")
	}
	...
	try {
		...
		try {
			creds = await authenticateWorkspace(workspace.id, ctx.apiKey, workspace.name ?? (basename(ctx.cwd) || "varth"), {
...
const SYNC_WIDGET_KEY = "varth-sync-progress"
const SPIN_FRAMES = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
const SPIN_MS = 100
