...
describe("buildSshOption", () => {
	it("composes the ssh command rsync's -e flag uses", () => {
		const got = buildSshOption({
			proxyCommand: "node /usr/local/lib/varth/teleport-proxy.js %h %p",
			knownHostsFile: "/tmp/known_hosts",
		})
		expect(got).toBe(
			"ssh -o ProxyCommand='node /usr/local/lib/varth/teleport-proxy.js %h %p' -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile='/tmp/known_hosts' -o BatchMode=yes -o ServerAliveInterval=15",
		)
	})
	...
	it("survives rsync's -e word splitter as one ProxyCommand token", () => {
		const got = buildSshOption({
			proxyCommand: "node /opt/varth/teleport-proxy.js %h %p",
			knownHostsFile: "/tmp/k/known_hosts",
		})
		const tokens = posixSplit(got)
		const proxyTokens = tokens.filter((t) => t.startsWith("ProxyCommand="))
		expect(proxyTokens).toEqual(["ProxyCommand=node /opt/varth/teleport-proxy.js %h %p"])
...
describe("buildRsyncArgv", () => {
	it("produces the expected argv (snapshot)", () => {
		...
		expect(argv).toEqual([
			...
			"/tmp/k/excludes",
			"-e",
			"ssh -o ProxyCommand='node /opt/varth/teleport-proxy.js %h %p' -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile='/tmp/k/known_hosts' -o BatchMode=yes -o ServerAliveInterval=15",
			"--delete",
			"/home/dev/project/",
