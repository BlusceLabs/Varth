...
beforeEach(async () => {
	tmp = await mkdtemp(join(tmpdir(), "varth-ssh-config-test-"))
	notifies.length = 0
})
...
describe("syncSshConfig", () => {
	it("writes the managed file under XDG_CONFIG_HOME with mode 0600", async () => {
		...
		const managed = join(tmp, "xdg", "varth", "ssh_config")
		const body = await readFile(managed, "utf8")
		expect(body).toContain("Host varth-alpha")
	...
	it("falls back to ~/.config when XDG_CONFIG_HOME is unset", async () => {
		const env = { HOME: tmp }
		await syncSshConfig([ws({ id: "w-1", name: "alpha", host: "a.example" })], makeCtx(), { env })
		const body = await readFile(join(tmp, ".config", "varth", "ssh_config"), "utf8")
		expect(body).toContain("Host varth-alpha")
	})
...
describe("ensureIncludeDirective", () => {
	it("creates ~/.ssh/config with the fenced block when the file does not exist", async () => {
		...
		expect(cfg).toContain("# >>> varth managed include >>>")
		expect(cfg).toContain(`Include ${join(tmp, "xdg", "varth", "ssh_config")}`)
		expect(cfg).toContain("# <<< varth managed include <<<")
	})
	...
	it("prepends the block to an existing file without touching user content", async () => {
		...
		expect(cfg.indexOf("# >>> varth managed include >>>")).toBeLessThan(cfg.indexOf("Host other.example"))
...
describe("applyIncludeBlock", () => {
	const block = "# >>> varth managed include >>>\nInclude /m\n# <<< varth managed include <<<"
	...
	it("replaces a mismatched block in place", () => {
		const stale = "# >>> varth managed include >>>\nInclude /old\n# <<< varth managed include <<<\n\nHost x\n"
