...
function makeHandler(run: CommandFn) {
	return async (args: string, ctx: ExtensionCommandContext): Promise<void> => {
		...
		const tctx: TeleportContext = {
			apiKey,
			endpoint: process.env.VARTH_REMOTE_ENDPOINT,
			cwd: ctx.cwd,
			signal: ctx.signal,
			ui: ctx.ui,
...
export default function teleportExtension(pi: ExtensionAPI): void {
	if (process.env.VARTH_EXPERIMENTAL_TELEPORT === undefined || process.env.VARTH_EXPERIMENTAL_TELEPORT === "") {
		return
	}
	pi.registerCommand("teleport", {
		description: "Open a varth PTY session in a sandbox workspace",
	...
	pi.registerCommand("remote-sessions", {
		description: "Browse workspaces and their varth sessions",
		handler: makeHandler(runRemoteSessions),
	})
	pi.registerCommand("ssh-config", {
		description: "Refresh ~/.config/varth/ssh_config from your workspaces",
