...
describe("autoInitFromEnv", () => {
	const originalVal = process.env.VARTH_AUTO_GIT_INIT

	afterEach(() => {
		if (originalVal === undefined) {
			process.env.VARTH_AUTO_GIT_INIT = undefined
		} else {
			process.env.VARTH_AUTO_GIT_INIT = originalVal
		}
	})

	it("returns false when VARTH_AUTO_GIT_INIT is not set", () => {
		process.env.VARTH_AUTO_GIT_INIT = undefined
		expect(autoInitFromEnv()).toBe(false)
	})

	it("returns true when VARTH_AUTO_GIT_INIT is '1'", () => {
		process.env.VARTH_AUTO_GIT_INIT = "1"
		expect(autoInitFromEnv()).toBe(true)
	})

	it("returns false when VARTH_AUTO_GIT_INIT is not '1'", () => {
		process.env.VARTH_AUTO_GIT_INIT = "yes"
		expect(autoInitFromEnv()).toBe(false)
	})
})
