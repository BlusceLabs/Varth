	...
	if (blockFlags.length > 0) {
		...
		if (retry > MAX_BLOCK_RETRIES || sameFailureRepeated) {
			...
			if (!escalationResponse.failed && escalationResponse.choice === "override") {
				...
				return toolErr(
					`Phase "${phase.name}" cannot complete — ${reasonNote}.\n\n${flagLines}${warnLines}\n\nFerment paused. An escalation artifact was written under .varth/ferments/${f.id}/escalations/phase-${phase.id}.json. The user must intervene before any further ferment tool calls.`,
				)
			}
		} else {
