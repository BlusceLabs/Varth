...
export default function fermentExtension(pi: ExtensionAPI, runtime: FermentRuntime = defaultFermentRuntime) {
	...
	// handler) and state.ts (VARTH_ACTIVE_FERMENT env-var reader); the
	...
	EntryTriggerRegistry.register("VARTH_ACTIVE_FERMENT", (event) => {
		if (event.kind !== "env-var") return { kind: "noop" }
		if (event.name !== "VARTH_ACTIVE_FERMENT") return { kind: "noop" }
		if (!event.value) return { kind: "noop" }
		return { kind: "enter-mode", mode: "ferment", reason: `VARTH_ACTIVE_FERMENT=${event.value}` }
	})
