		...
		promise
			.then(async (nextState) => {
				...
				try {
					...
					const { strategy, bm25K1, bm25B, fieldWeights } = varthConfig.mcpSearch
	...
	if (shouldRegisterProxyTool) {
		pi.registerTool({
			...
			async execute(
				...
				try {
					const varthConfig = loadConfig()
					maxToolResultChars = varthConfig.maxToolResultChars
				} catch {
				...
				try {
					const varthConfig = loadConfig()
					mcpSearchLimit = varthConfig.mcpSearchLimit
				} catch {
