 ...
 * `VARTH_CODING_AGENT_DIR` and re-imports the cache module so the memoized
 ...
 * `VARTH_CODING_AGENT_DIR`.
 */
async function buildHarness(): Promise<Harness> {
	tempHome = mkdtempSync(join(tmpdir(), "mcp-cache-test-"))
	process.env.VARTH_CODING_AGENT_DIR = tempHome
...
beforeEach(() => {
	originalEnv = process.env.VARTH_CODING_AGENT_DIR
})

afterEach(() => {
	if (originalEnv === undefined) delete process.env.VARTH_CODING_AGENT_DIR
	else process.env.VARTH_CODING_AGENT_DIR = originalEnv
	if (tempHome) {
