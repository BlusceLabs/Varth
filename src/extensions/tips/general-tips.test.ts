...
describe("GENERAL_TIPS", () => {
	it("uses concrete Varth workflows instead of generic placeholders", () => {
		...
		expect(messages).toContain("Resume the latest session with `varth --continue`.")
		expect(messages).toContain("Use `varth --verbose` when output looks off.")
	...
	it("fits every built-in tip in an 80-column row without truncation", () => {
		for (const tip of GENERAL_TIPS) {
			const [line] = renderTipRow({ ...tip, source: "varth.general" } as TipCandidate, plainTheme, 80)
