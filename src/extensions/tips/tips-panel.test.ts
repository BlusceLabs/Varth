...
function makeTip(id: string, message: string, source = "varth.general"): TipCandidate {
...
describe("sourceToLabel", () => {
	it("maps known sources", () => {
		expect(sourceToLabel("varth.general")).toBe("General")
		expect(sourceToLabel("varth.ferment")).toBe("Ferment")
	})

	it("strips varth prefix and capitalizes unknown sources", () => {
		expect(sourceToLabel("varth.custom")).toBe("Custom")
	})

	it("capitalizes sources without varth prefix", () => {
...
describe("createTipsPanel", () => {
	describe("render", () => {
		it("renders numbered tips grouped by source", () => {
			const tips = [
				makeTip("a", "First tip.", "varth.general"),
				makeTip("b", "Second tip.", "varth.general"),
				makeTip("c", "Ferment tip.", "varth.ferment"),
			]
	...
	describe("grouping", () => {
		it("groups tips from different providers under separate headers", () => {
			const tips = [
				makeTip("a", "General tip.", "varth.general"),
				makeTip("b", "Ferment tip.", "varth.ferment"),
				makeTip("c", "Custom tip.", "varth.custom"),
			]
			const panel = createTipsPanel(tips, plainTheme(), makeTui(), vi.fn())
		...
		it("hides empty groups when search filters out all their tips", () => {
			const tips = [
				makeTip("a", "Press shift to do things.", "varth.general"),
				makeTip("b", "Ferment specific tip.", "varth.ferment"),
			]
			const panel = createTipsPanel(tips, plainTheme(), makeTui(), vi.fn())
