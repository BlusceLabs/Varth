...
describe("Claude Code hook adapter integration", () => {
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "varth-claude-code-hook-adapter-"))
...
if (process.env.VARTH_HOOK_EVENT !== "PreToolUse") process.exit(2)
if (process.env.VARTH_TOOL_NAME !== "Bash") process.exit(2)
...
`,
		...
		expect(pi.sendMessage).toHaveBeenCalledWith(
			expect.objectContaining({
				...
				customType: "varth-claude-code-hook-context",
				display: false,
			}),
