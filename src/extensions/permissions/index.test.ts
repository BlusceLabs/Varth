...
describe("plan mode assumption detection", () => {
	...
	it("Start as ferment persists a ferment artifact under .varth/ferments", async () => {
		const harness = createPermissionsHarness(["read", "bash"], { plan: true })
		await harness.fire("session_start", {}, createMockContext([]))
		...
		try {
			...
			const fermentsDir = join(tmpDir, ".varth", "ferments")
	...
	it("Start as ferment applies the implementation-ferment tool profile", async () => {
		...
		// The START_AS_FERMENT branch persists a ferment artifact under <cwd>/.varth/ferments
...
describe("subagent inherits parent session permission mode", () => {
	...
	it("reads mode from VARTH_PERMISSIONS_<parentSessionId> when VARTH_PARENT_SESSION_ID is set", async () => {
	...
	it("parent session key takes precedence over base VARTH_PERMISSIONS", async () => {
	...
	it("falls back to config default when neither VARTH_PERMISSIONS nor parent session key is set", async () => {
		Reflect.deleteProperty(process.env, PERMISSIONS_ENV_KEY)
		Reflect.deleteProperty(process.env, PARENT_SESSION_ID_ENV_KEY)
