 ...
 *
 * Both prefer varth-dev models, walking tiers light → standard → heavy.
 * Only when no varth-dev model is available do they fall back to the
...
export function resolveClassifierModels(modelRegistry: ModelRegistry): ClassifierModels | undefined {
	const primary = resolveVarthModel(modelRegistry) ?? cheapestAvailable(modelRegistry)
	if (!primary) return undefined

	const fallback = resolveVarthModel(modelRegistry, primary.id) ?? cheapestAvailable(modelRegistry, primary.id)

	return { primary, fallback }
}

/**
 * Find the best available varth-dev model, excluding `excludeId`.
 * Walks tiers light → standard → heavy and returns the first non-empty tier's
 * cheapest model.
 */
function resolveVarthModel(modelRegistry: ModelRegistry, excludeId?: string): Model<Api> | undefined {
	for (const tier of TIERS_ASCENDING) {
		for (const [id, caps] of MODEL_CAPABILITIES) {
			...
			const resolved = modelRegistry.find(VARTH_DEV_PROVIDER, id)
