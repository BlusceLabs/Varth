...
beforeEach(() => {
	tmpCwd = mkdtempSync(join(tmpdir(), "varth-perm-test-"))
})
...
describe("loadConfig merging", () => {
	it("reads project config and tags rules by source", () => {
		mkdirSync(join(tmpCwd, ".varth"), { recursive: true })
		writeFileSync(
			join(tmpCwd, ".varth", "permissions.json"),
			JSON.stringify({
				defaultMode: "plan",
				allow: ["bash(git:*)"],
				deny: ["write(.env)"],
	...
	it("merges local on top of project", () => {
		...
		writeFileSync(
			join(tmpCwd, ".varth", "permissions.local.json"),
	...
	it("cli override replaces merged config entirely", () => {
		mkdirSync(join(tmpCwd, ".varth"), { recursive: true })
		writeFileSync(join(tmpCwd, ".varth", "permissions.json"), JSON.stringify({ allow: ["bash(git:*)"] }))
	...
	it("accepts yolo as defaultMode and round-trips it", () => {
		mkdirSync(join(tmpCwd, ".varth"), { recursive: true })
		writeFileSync(join(tmpCwd, ".varth", "permissions.json"), JSON.stringify({ defaultMode: "yolo" }))
