...
/**
 * Check whether a file path is within .varth/plans/ relative to cwd.
 * Accepts both relative paths (starting with .varth/plans/) and
 * absolute paths under <cwd>/.varth/plans/.
 * Path traversal is prevented by resolving to an absolute path first.
 */
export function isWithinVarthPlans(filePath: string, cwd: string): boolean {
	const normalizedCwd = cwd.endsWith("/") ? cwd : `${cwd}/`
	const plansDir = `${normalizedCwd}.varth/plans/`
...
// `set_phase` is a varth built-in. `agent`/`get_subagent_result`/`steer_subagent`
...
export default function permissionsExtension(pi: ExtensionAPI): void {
	...
	// VARTH_PERMISSIONS_<sessionId> and advertises the session ID via
	// VARTH_PARENT_SESSION_ID. If this variable is absent, we fall back to the
	// base key.
	...
	pi.on("turn_end", async (event, ctx) => {
		...
		if (choice === EXECUTE) {
			...
			try {
				...
				// lands in the project's .varth/ferments/ directory, not process.cwd().
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		// Plan persona path-scope enforcement: when VARTH_AGENT_PERSONA=plan (case-insensitive),
		// write and edit are only allowed for .varth/plans/* paths.
		if (process.env.VARTH_AGENT_PERSONA?.toLowerCase() === "plan") {
			if (toolName === "write" || toolName === "edit") {
				const filePath =
					typeof input.file_path === "string" ? input.file_path : typeof input.path === "string" ? input.path : ""
				if (filePath && !isWithinVarthPlans(filePath, ctx.cwd)) {
					return {
						block: true,
						reason: `Plan persona: ${toolName} is restricted to .varth/plans/ files. The path "${filePath}" is outside that scope.`,
					}
				}
				// path is within .varth/plans/ — allow without further checks
				return undefined
			}
		}
