...
describe("prompt enrichment Claude Code skills", () => {
	...
	beforeEach(() => {
		vi.restoreAllMocks()
		dir = mkdtempSync(join(tmpdir(), "varth-prompt-claude-skills-"))
		oldAgentDir = process.env.VARTH_CODING_AGENT_DIR
		oldHome = process.env.HOME
		oldXdgCacheHome = process.env.XDG_CACHE_HOME
		process.env.VARTH_CODING_AGENT_DIR = join(dir, "agent")
	...
	afterEach(() => {
		if (oldAgentDir === undefined) {
			// biome-ignore lint/performance/noDelete: process.env requires delete to truly unset.
			delete process.env.VARTH_CODING_AGENT_DIR
		} else {
			process.env.VARTH_CODING_AGENT_DIR = oldAgentDir
		}
	...
	it("contributes Varth project skills through resources_discover", async () => {
		const cwd = join(dir, "project", "src")
		const projectSkillPath = join(dir, "project", ".varth", "skills")
		writeSkill(join(projectSkillPath, "typescript-safety", "SKILL.md"), {
			description: "Use Varth project TypeScript patterns.",
		})
	...
	it("injects Varth project skills without configured paths", async () => {
		const cwd = join(dir, "project", "src")
		writeSkill(join(dir, "project", ".varth", "skills", "typescript-safety", "SKILL.md"), {
			description: "Use Varth project TypeScript patterns.",
		})
		const { beforeAgentStart } = buildPromptExtensionWithHandlers([])
		if (!beforeAgentStart) throw new Error("before_agent_start handler was not registered")
		...
		expect(result.systemPrompt).toContain("<available_skills>")
		expect(result.systemPrompt).toContain("<name>typescript-safety</name>")
		expect(result.systemPrompt).toContain("Use Varth project TypeScript patterns")
	})
	...
	it("keeps home-relative configured skill paths in the prompt", async () => {
		const cwd = join(dir, "project")
		const configuredSkills = ".config/varth/harness/skills"
...
describe("model role startup warnings", () => {
	...
	it("keeps unavailable role warnings when Varth auth is already configured", () => {
...
describe("deprecated model notification", () => {
	...
	function makeMockContext(modelId: string | undefined, sessionId = "test-session-1") {
		return {
			cwd: "/tmp",
			model: modelId ? { id: modelId, provider: "varth-dev" } : undefined,
			hasUI: true,
			sessionManager: {
