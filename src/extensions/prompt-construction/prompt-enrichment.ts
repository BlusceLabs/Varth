 ...
 * (detected via Agent worker context or the legacy VARTH_SUBAGENT env var).
 *
 * Main model mode:
...
import { getVarthProjectSkillPaths } from "../../skill-paths.js"
...
const HARNESS_SETTINGS_PATH = join(homedir(), ".config", "varth", "harness", "settings.json")
...
function writeMultiModelSetting(enabled: boolean): void {
	try {
		...
		const dir = join(homedir(), ".config", "varth", "harness")
...
export default function (skillPaths: string[]) {
	return (pi: ExtensionAPI) => {
		...
		pi.registerFlag("debug-prompts", {
			...
			default: process.env.VARTH_DEBUG_PROMPTS === "1",
		})
		...
		pi.on("resources_discover", (event) => {
			const skillPaths = getVarthProjectSkillPaths(event.cwd)
		...
		pi.on("before_agent_start", async (event, ctx) => {
			...
			if (cachedSkills === undefined) {
				...
				const allSkillPaths = Array.from(
					new Set([
						...getVarthProjectSkillPaths(ctx.cwd),
			...
			const env: EnvironmentInfo = {
				...
				cwd: ctx.cwd,
				documentsDir: join(ctx.cwd, ".varth", "docs"),
			...
			// e.g. orchestrators embedding varth as a managed agent.
			const appendSystemPrompt = event.systemPromptOptions?.appendSystemPrompt?.trim()
			if (appendSystemPrompt) {
				systemPrompt = `${systemPrompt}\n\n${appendSystemPrompt}`
			}

			const debugSession = process.env.VARTH_DEBUG_SESSION
			const debugFlag = pi.getFlag("debug-prompts") === true
			if (debugFlag || debugSession) {
				const sessionId = debugSession ?? randomUUID().slice(0, 8)
				process.env.VARTH_DEBUG_PROMPTS = "1"
				process.env.VARTH_DEBUG_SESSION = sessionId

				const debugDir = join(ctx.cwd, ".varth", "debug", sessionId)
				...
				process.env.VARTH_DEBUG_PROMPTS = undefined
				process.env.VARTH_DEBUG_SESSION = undefined
			}
