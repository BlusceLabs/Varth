...
/**
 * Startup-time check for new varth versions. Uses cache (24h) and displays
...
export default function startupUpdateExtension(pi: ExtensionAPI) {
	pi.on("session_start", async (_event, ctx: ExtensionContext) => {
		...
		// stable. Currency on canary is checked by `varth update --canary`.
		if (parseCanarySha7(current) !== null) return
		try {
			const result = await checkForUpdate({ currentVersion: current, skipCache: false })
			if (result.hasUpdate) {
				const updateCmd = ctx.ui.theme.bold(isHomebrewInstall() ? "brew upgrade varth" : "varth update")
