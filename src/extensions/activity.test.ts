...
describe("ActivityBus – send and broadcast", () => {
	...
	it("broadcasts JSON events as NDJSON lines to connected clients", async () => {
		...
		const { received, client: c, connected } = connectClient("/tmp/varth/sess-broadcast.sock")
		client = c
		await connected
...
describe("ActivityBus – lifecycle", () => {
	...
	it("cleans up socket file on stop()", async () => {
		...
		expect(existsSync("/tmp/varth/sess-cleanup.sock")).toBe(true)
		await bus.stop()
		expect(existsSync("/tmp/varth/sess-cleanup.sock")).toBe(false)
	})
	...
	it("sends session_shutdown before closing on stop()", async () => {
		...
		const { received, client, connected } = connectClient("/tmp/varth/sess-shutdown-msg.sock")
		await connected
...
describe("ActivityBus – incoming NDJSON no-op", () => {
	...
	it("does not crash on malformed or valid incoming data", async () => {
		...
		client = net.createConnection("/tmp/varth/sess-incoming.sock")
		const c = client
