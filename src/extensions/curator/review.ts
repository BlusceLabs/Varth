...
import { spawnVarthSubprocess } from "../../utils/spawn-varth-subprocess.js"
...
export function buildCuratorPrompt(candidates: CuratorCandidate[]): string {
	...
	return `You are the Varth skill curator. Your job is **consolidation only** — not gap-finding, not creating new skills from scratch.
...
function spawnReviewAgent(
	...
	return spawnVarthSubprocess({
		args,
		stdout: opts?.stdout,
		stderr: opts?.stderr,
		detached: opts?.detached,
		env: { VARTH_SUBAGENT: "1", VARTH_SESSION_REVIEW: "1" },
	})
}
...
export function debugLog(msg: string): void {
	const path = process.env.VARTH_REVIEW_LOG
	if (!path) return
...
export function spawnSessionReview(opts: RunSessionReviewOptions): void {
	...
	const reviewLogPath = process.env.VARTH_REVIEW_LOG
