...
export default function curatorExtension(pi: ExtensionAPI, options?: CuratorExtensionOptions): void {
	...
	const skillsDir = options?.skillsDir ?? join(homedir(), ".config", "varth", "harness", "skills")
	...
	// Capture provider/model from the first real LLM request — works regardless of how varth is invoked.
	...
	const SESSION_REVIEW_THRESHOLD = Number(process.env.VARTH_REVIEW_THRESHOLD ?? 5)
			...
			if (!providerModel) {
				return {
					content: [
						{
							type: "text" as const,
							text: "Curator: no provider/model configured. Start varth with --provider and --model.",
						},
					],
					details: null,
				}
