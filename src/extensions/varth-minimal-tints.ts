// Apply per-process dynamic background tints for the `varth-minimal` theme.
//
// We mutate the loaded Theme's bgColors Map directly instead of writing tints
// into the on-disk theme file. Why: when varth runs simultaneously in two
...
export default function varthMinimalTintsExtension(pi: ExtensionAPI) {
	...
	const triggerRerender = (ctx: ExtensionContext) => {
		...
		ctx.ui.setStatus("varth-tints-rerender", undefined)
	}
	...
	pi.on("session_start", (_event, ctx: ExtensionContext) => {
		if (getActiveThemeName() === "varth-minimal") applyTints(ctx)

		unsubscribeThemeChange?.()
		unsubscribeThemeChange = onThemeChange((newName) => {
			if (newName === "varth-minimal") {
				applyTints(ctx)
				triggerRerender(ctx)
			}
		})
