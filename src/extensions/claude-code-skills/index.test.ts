...
describe("Claude Code skills extension", () => {
	beforeEach(() => {
		dir = mkdtempSync(join(tmpdir(), "varth-claude-code-skill-tool-"))
	...
	it("does not load native skills through the Claude-compatible Skill tool", async () => {
		writeSkill(
			join(dir, "project", ".varth", "skills", "best-practices", "SKILL.md"),
			"Varth project skill instructions.",
		)
		writeSkill(
		...
		expect(textResult(result)).not.toContain("Varth project skill instructions.")
	...
	it("contributes sanitized Claude Code skills through resources_discover", async () => {
		...
		expect(result).toMatchObject({
			skillPaths: [expect.stringContaining("varth-claude-code-skills-")],
		})
