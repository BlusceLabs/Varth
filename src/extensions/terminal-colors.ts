...
// Any theme without these values (including varth-minimal) lets the terminal
// own its bg/fg.
//
...
// theme bg lingers under another theme or varth-minimal.
//
...
function getThemeColors(themeName: string): { fgHex: string; bgHex: string } | null {
	try {
		const dir = process.env.VARTH_CODING_AGENT_DIR
		if (!dir) return null
...
export default function terminalColorsExtension(pi: ExtensionAPI) {
	...
	const reactToThemeChange = (newName: string | undefined) => {
		...
		lastCtx?.ui.setStatus("varth-theme-rerender", undefined)
	}
	...
	pi.on("session_start", (_event, ctx) => {
		...
		ctx.ui.setWidget("varth-osc-sensor", () => ({
			invalidate(): void {
