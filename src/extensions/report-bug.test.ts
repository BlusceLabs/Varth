...
describe("reportBugExtension", () => {
	...
	it("registers the 'bug' command", () => {
		...
		expect(commands.get("bug")?.description).toBe("Report a bug in varth — opens GitHub issue form")
	})
	...
	it("builds URL with required params when called without args (UI mode)", async () => {
		...
		expect(url).toMatch(/^https:\/\/github\.com\/getvarth\/varth\/issues\/new\?/)
	...
	it("falls back to console.log in headless mode without opening browser", async () => {
		...
		expect(console.log).toHaveBeenCalledWith(expect.stringContaining("https://BlusceLabs/varth/issues/new"))
