...
const DEBUG_FLAG = process.env.VARTH_TELEMETRY_DEBUG

function logEventToFile(event: string, properties: Record<string, unknown>): void {
	if (!DEBUG_FLAG) return
	try {
		const logDir = path.join(process.cwd(), ".varth")
...
function resourceAttributes() {
	return [strAttr("service.name", "varth"), strAttr("user_agent.original", `varth/${getVersion()}`)]
}
