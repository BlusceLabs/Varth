...
This extension collects usage data for the Varth CLI and sends it via OTLP (OpenTelemetry Protocol).
...
| Resource attributes | `service.name="varth"`, `user_agent.original="varth/{version}"` |
| Scope | `name="varth"`, `version="1.0.0"` |
...
- Telemetry is on by default and controlled by `telemetry.enabled` in `~/.config/varth/config.json` (overridable via `$VARTH_TELEMETRY_ENABLED`).
