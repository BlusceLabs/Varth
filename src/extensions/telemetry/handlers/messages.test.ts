...
describe("handleMessageEnd", () => {
	...
	it("resolves provider via varth-dev to ai-enabler", async () => {
		const ctx = makeCtx()
		const emitSpy = vi.spyOn(ctx, "emit")
...
describe("handleBeforeAgentStart", () => {
	...
	it("emits varth.user_message with message_length and model", () => {
		const ctx = makeCtx()
		ctx.currentModel = "claude-3-5-sonnet"
		const emitSpy = vi.spyOn(ctx, "emit")
...
describe("handleAgentEnd", () => {
	...
	it("emits varth.error when last message is a toolResult with isError=true", () => {
		const ctx = makeCtx()
		ctx.currentModel = "claude-3-5-sonnet"
		const emitSpy = vi.spyOn(ctx, "emit")
