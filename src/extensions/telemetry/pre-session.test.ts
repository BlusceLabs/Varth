...
const testHeaders = { Authorization: "Bearer test-key", "User-Agent": "varth/1.0.0-test" }
...
describe("sendPreSessionEvent", () => {
	...
	it("is a no-op when no Authorization header is present", async () => {
		sendPreSessionEvent(makeConfig({ headers: { "User-Agent": "varth/1.0.0" }, apiKey: "" }), "app_started")
		await drain()
