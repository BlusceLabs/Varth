...
export default function modelSwitchExtension(pi: ExtensionAPI) {
	pi.registerTool({
		...
		label: "Switch Model",
		description:
			'Change the active AI model to a different one. Provide the model in provider/id format, e.g. "varth-dev/kimi-k2.6". Uses pi.setModel() internally.',
		parameters: Type.Object({
			model: Type.String({
				description:
					'Target model identifier in "provider/modelId" format (e.g. "varth-dev/kimi-k2.6", "anthropic/claude-sonnet-4-20250514").',
			}),
		}),
	...
	pi.on?.("model_select", async (event, ctx) => {
		...
		// process.__varthMultiModelEnabled.  getMultiModelEnabled() detects a
