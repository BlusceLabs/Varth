 ...
 *  Priority: $OLLAMA_HOST > $VARTH_OLLAMA_HOST > http://localhost:11434.
...
export function resolveOllamaHost(): string {
	const fromEnv = process.env.OLLAMA_HOST?.trim() || process.env.VARTH_OLLAMA_HOST?.trim()
 ...
 *  (including the varth-dev and varth-experimental slots managed by
 ...
 *
 * Non-Varth providers already in models.json are preserved.
 */
