...
describe("discoverCommandFiles", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "varth-cmd-test-"))
	})
...
describe("discoverSubdirectoryCommandFiles", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "varth-subcmd-test-"))
	})
...
describe("migrateCommandToPrompt", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "varth-migrate-test-"))
...
describe("runSetupWizard", () => {
	...
	it("returns a cancelled result when optional custom skill input is cancelled", async () => {
		clackMock.multiselect.mockResolvedValueOnce([".config/varth/harness/skills"])
	...
	it("marks successful setup results as not cancelled", async () => {
		...
		expect(result).toEqual({
			cancelled: false,
			skillPaths: [".config/varth/harness/skills"],
