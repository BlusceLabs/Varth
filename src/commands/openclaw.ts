...
export async function runOpenClaw(args: string[]): Promise<number> {
	...
	try {
		...
		if (!tool) {
			console.error("varth openclaw: integration not registered")
			return 1
		}
		await tool.write(scope, prepped.apiKey, prepped.models)
		console.log("varth openclaw: configuration written.")
		return 0
	} catch (err) {
		console.error(`varth openclaw: ${(err as Error).message}`)
		return 1
	}
}
