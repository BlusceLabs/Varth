...
/**
 * `varth claude [args]` — launch Claude Code with varth env vars injected
 * for this run only (no disk write). The user's ~/.claude/settings.json is
 * left untouched; persistent install lives behind `varth setup` (or a
 * future `varth claude --persist`).
 *
 * All args after `claude` are forwarded to the binary verbatim — that's how
 * `varth claude --help`, `varth claude /resume`, etc. work without us
...
export async function runClaude(args: string[]): Promise<number> {
	...
	try {
		...
		console.error(`varth claude: ${(err as Error).message}`)
		return 1
	}
}
