...
const UPDATE_USAGE = [
	"Usage: varth update [source|self|pi] [--self] [--extensions] [--extension <source>] [--canary] [--force] [--dry-run]",
	"",
	"  source        Update one installed Pi package, e.g. context-mode or npm:context-mode",
	"  self, pi      Update Varth itself only",
	"  --self        Update Varth itself only",
	...
	"  --dry-run     Check Varth self-updates without installing",
].join("\n")
...
function parseFlags(args: string[]): UpdateFlags | string {
	...
	if ((flags.canary || flags.dryRun) && packageTarget) {
		return "--canary and --dry-run can only be used when updating Varth itself"
	}
...
/**
 * `varth update` — explicit self-update entry point. Always skips the
 * cached state so users get fresh results when they ask.
 *
 * On Linux/macOS the swap is atomic via POSIX rename(2); on Windows we
 * rotate varth.exe → varth.exe.old and the user is told to restart
 * their terminal.
 */
export async function runUpdate(args: string[]): Promise<number> {
	...
	if (typeof parsed === "string") {
		...
		console.error(`varth update: ${parsed}`)
		return 2
	}
	const flags = parsed
...
async function updatePackages(source: string | undefined): Promise<number> {
	...
	if (packages.length === 0) {
		console.log("varth packages: none installed")
		return 0
	}

	const updateSource = source ? resolvePackageSource(source, packages) : undefined
	if (source && !updateSource) {
		console.error(`varth update: no matching package found for ${source}`)
		return 1
	}
	...
	try {
		...
		console.error(`varth update: package update failed — ${(err as Error).message}`)
		return 1
	}
...
async function updateSelf(flags: Pick<UpdateFlags, "canary" | "dryRun" | "force">): Promise<number> {
	...
	if (isHomebrewInstall()) {
		if (flags.canary) {
			console.log("varth is managed by Homebrew. Canary builds are not published to Homebrew.")
			console.log("")
			console.log("To use canary, uninstall the Homebrew package and reinstall directly:")
			console.log("")
			console.log("  brew uninstall varth")
			console.log("  curl -fsSL https://BlusceLabs/varth/releases/latest/download/install.sh | bash")
			console.log("")
			console.log("Then re-run: varth update --canary")
			return 0
		}
		console.log("varth is managed by Homebrew. Use Homebrew to update:")
		console.log("")
		console.log("  brew upgrade varth")
		console.log("")
		console.log("If you want the self-update behaviour, install varth outside of Homebrew.")
		return 0
	}
	...
	try {
		...
		console.error(`varth update: failed to check for updates: ${(err as Error).message}`)
		return 1
	}

	if (!check.hasUpdate) {
		console.log(`varth: already up to date (${current})`)
		return 0
	}
	if (flags.dryRun) {
		console.log(`varth update available: ${current} → ${check.latestVersion}`)
	...
	if (!flags.force) {
		...
		// pipelines run `varth update --force`).
		const ok = await confirm(`Varth update available: ${current} → ${check.latestVersion}\nUpdate? [Y/n]: `)
		if (!ok) {
	...
	try {
		...
		console.error(`varth update: update failed — ${(err as Error).message}`)
		console.error("")
		console.error("Please copy the error message above and create a bug report at https://BlusceLabs/varth/issues")
		return 1
	}

	if (process.platform === "win32") {
		console.log(`✓ varth installed: ${check.latestVersion}`)
		console.log("Restart your terminal to use the new version.")
	} else {
		console.log(`✓ varth updated to ${check.latestVersion}`)
	}
	return 0
}
