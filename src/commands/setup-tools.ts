...
/**
 * `varth setup-tools` — interactive wizard to configure multiple coding
 * tools (Cursor, OpenCode, Claude Code, OpenClaw, GSD2) in one pass.
 *
 * This command extracts the "tools" step that used to be part of
 * `varth setup`, so users can re-run tool configuration without going
...
export async function runSetupTools(args: string[]): Promise<number> {
	...
	if (!apiKey) {
		console.error("varth: no API key configured. Run `varth setup` or set $VARTH_API_KEY.")
		return 1
	}

	intro("varth setup-tools")
	...
	// `varth setup`, `varth config telemetry`, or $VARTH_TELEMETRY_ENABLED)
	...
	// Fetch live models.
	const agentDir = process.env.VARTH_CODING_AGENT_DIR ?? resolve(process.env.HOME ?? "~", ".config/varth-coding-agent")
