...
describe("runUpdate flag parsing", () => {
	...
	it("--help documents --canary", async () => {
		...
		expect(out).toContain("Usage: varth update")
	})
	...
	it("rejects global Pi flags that are not valid for update", async () => {
		const code = await runUpdate(["--model", "varth-dev/kimi-k2.6"])
		expect(code).toBe(2)
...
describe("runUpdate Homebrew branch", () => {
	...
	it("prints canary-specific message on Homebrew + --canary and skips download", async () => {
		...
		expect(out).toContain("brew uninstall varth")
		expect(out).toContain("install.sh")
		expect(out).toContain("varth update --canary")
		expect(out).not.toContain("brew upgrade varth")
	...
	it("prints generic Homebrew message on bare update (no --canary)", async () => {
		...
		expect(code).toBe(0)
		const out = logSpy.mock.calls.map((c) => String(c[0] ?? "")).join("\n")
		expect(out).toContain("brew upgrade varth")
		expect(out).not.toContain("brew uninstall varth")
		expect(checkForUpdateMock).not.toHaveBeenCalled()
	})
})
...
describe("runUpdate package targets", () => {
	...
	it("updates all packages before checking Varth self-updates on bare update", async () => {
		const code = await runUpdate([])
		...
		expect(out).toContain("varth: already up to date")
	})
	...
	it("keeps --dry-run as a Varth self-update check", async () => {
