...
const TELEMETRY_ENV = "VARTH_TELEMETRY_ENABLED"

/**
 * `varth config telemetry [on|off]` — show or set telemetry.enabled in
 * config.json. With no value, prints the current state (and notes when
 * an env-var override is in effect).
 *
 * Anything else under `varth config <subcommand>` is unrecognised; we
...
export async function runConfig(args: string[]): Promise<number> {
	...
	switch (sub) {
		...
		default:
			console.error(`varth config: unknown subcommand "${sub}"`)
			printUsage()
			return 2
	}
}
...
function handleTelemetry(args: string[]): number {
	...
	if (enabled === null) {
		console.error(`varth config telemetry: expected "on" or "off", got "${args[0]}"`)
		return 2
	}
...
function printUsage(): void {
	console.error("Usage: varth config telemetry [on|off]")
	console.error("       varth config telemetry           # show current status")
}
