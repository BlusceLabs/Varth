...
/**
 * Resolve the varth API key from $VARTH_API_KEY first, then the config
 * file. Returns null when neither is set; callers print a friendly "run
 * varth setup" message rather than throwing.
 */
export function resolveApiKey(): string | null {
	const env = process.env.VARTH_API_KEY
...
export async function prepareTool(
	...
	if (!tool) {
		...
		console.error(`varth: integration "${toolId}" not registered.`)
		return null
	}
