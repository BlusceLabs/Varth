...
describe("resolveAuxiliaryFilesDir", () => {
	...
	it("returns $XDG_DATA_HOME/varth/ when XDG_DATA_HOME is set and PI_PACKAGE_DIR is not", () => {
		const env = { XDG_DATA_HOME: "/xdg/data" }
		expect(resolveAuxiliaryFilesDir(env, home)).toBe("/xdg/data/varth")
	})

	it("returns ~/.local/share/varth/ when neither PI_PACKAGE_DIR nor XDG_DATA_HOME is set", () => {
		const env: Record<string, string | undefined> = {}
		expect(resolveAuxiliaryFilesDir(env, home)).toBe("/home alice/.local/share/varth")
	})
	...
	describe("binary-sibling share directory", () => {
		...
		beforeEach(() => {
			tmpBase = join(tmpdir(), `varth-resolver-test-${process.pid}`)
			// Simulate dist/bin/varth + dist/share/varth/package.json
			mkdirSync(join(tmpBase, "bin"), { recursive: true })
			mkdirSync(join(tmpBase, "share", "varth"), { recursive: true })
			writeFileSync(join(tmpBase, "share", "varth", "package.json"), "{}")
		})
		...
		it("returns sibling share dir when package.json exists next to the binary", () => {
			const execPath = join(tmpBase, "bin", "varth")
			const env: Record<string, string | undefined> = {}
			expect(resolveAuxiliaryFilesDir(env, home, execPath)).toBe(join(tmpBase, "share", "varth"))
		})
		...
		it("falls through to XDG when sibling share dir has no package.json", () => {
			rmSync(join(tmpBase, "share", "varth", "package.json"))
			const execPath = join(tmpBase, "bin", "varth")
			const env = { XDG_DATA_HOME: "/xdg/data" }
			expect(resolveAuxiliaryFilesDir(env, home, execPath)).toBe("/xdg/data/varth")
		})
	})
})
