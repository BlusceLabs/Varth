...
describe("assetName", () => {
	it("formats `<binary>_<os>_<arch>.tar.gz` on Linux/macOS", () => {
		...
		try {
			expect(assetName(REPO)).toBe("varth_linux_amd64.tar.gz")
		} finally {
	...
	it("uses .zip on Windows (different release format)", () => {
		...
		Object.defineProperty(process, "arch", { value: "x64" })
		try {
			expect(assetName(REPO)).toBe("varth_windows_amd64.zip")
		} finally {
...
describe("GitHubClient.fetchChecksum", () => {
	it("finds the checksum line for our platform's asset", async () => {
		...
		const text = [
			"9999999999999999999999999999999999999999999999999999999999999999  varth_darwin_arm64.tar.gz",
			"deadbeef00000000000000000000000000000000000000000000000000000000  varth_linux_amd64.tar.gz",
		].join("\n")
		const fetchImpl = vi.fn().mockResolvedValue({
			ok: true,
			status: 200,
	...
	it("throws when our asset isn't listed", async () => {
		const fetchImpl = vi.fn().mockResolvedValue({
			ok: true,
			status: 200,
			text: async () => "0000  varth_other_other.tar.gz\n",
		}) as unknown as typeof fetch
		const client = new GitHubClient({ fetch: fetchImpl })
