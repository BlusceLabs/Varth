...
import { GitHubClient, type Repo, VARTH_REPO } from "./github.js"
 ...
 * $VARTH_NO_UPDATE_CHECK by short-circuiting to a "no update" result.
 *
...
export async function checkForUpdate(opts: CheckOptions): Promise<CheckResult> {
	const repo = opts.repo ?? VARTH_REPO
 ...
 *
 * On Windows the install rotates `varth.exe` → `varth.exe.old` and drops
...
export async function applyUpdate(opts: UpdateOptions): Promise<UpdateResult> {
	...
	const workDir = mkdtempSync(join(tmpdir(), "varth-update-"))
	try {
		...
		try {
			...
			const shareSrc = join(extractedRoot, "share", "varth")
