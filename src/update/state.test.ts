...
describe("update state cache", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-update-state-test-"))
		prevHome = process.env.HOME
		prevXdg = process.env.XDG_CACHE_HOME
		// statePath() consults XDG_CACHE_HOME first; pin it at the temp dir
		// so the test never touches the developer's real ~/.cache/varth.
	...
	function statePathFor(): string {
		return join(tmp, "varth", "state.json")
	}

	function writeRawState(json: unknown): void {
		const path = statePathFor()
		mkdirSync(join(tmp, "varth"), { recursive: true })
		writeFileSync(path, JSON.stringify(json, null, 2))
	}

	it("returns null for a missing state file", () => {
		expect(loadRepoState("castai", "varth")).toBeNull()
	})

	it("round-trips per-repo state through saveRepoState + loadRepoState", () => {
		saveRepoState("castai", "varth", {
			checked_at: "2023-11-14T22:13:20.000Z",
			latest_version: "1.2.3",
			release_url: "https://github.com/castai/varth/releases/tag/v1.2.3",
		})
		const got = loadRepoState("castai", "varth")
		expect(got).toEqual({
			checked_at: "2023-11-14T22:13:20.000Z",
			latest_version: "1.2.3",
			release_url: "https://github.com/castai/varth/releases/tag/v1.2.3",
		...
		writeRawState({
			repos: {
				"castai/varth-dev": {
					...
					release_url: "https://github.com/castai/varth-dev/releases/tag/v0.0.22",
				},
			},
		})
		const got = loadRepoState("castai", "varth-dev")
		expect(got?.latest_version).toBe("v0.0.22")
		expect(got?.release_url).toBe("https://github.com/castai/varth-dev/releases/tag/v0.0.22")
		...
		writeRawState({
			repos: {
				"castai/varth": {
					...
					release_url: "https://github.com/castai/varth/releases/tag/v0.1.52",
				},
			},
		})
		saveRepoState("castai", "varth-dev", { checked_at: "2023-11-14T22:13:20.000Z", latest_version: "v0.0.22" })
		const raw = JSON.parse(readFileSync(statePathFor(), "utf-8"))
		expect(raw.repos["castai/varth"]).toEqual({
			checked_at: "2026-05-06T10:05:19+03:00",
			latest_version: "v0.1.52",
			release_url: "https://github.com/castai/varth/releases/tag/v0.1.52",
		})
		expect(raw.repos["castai/varth-dev"]).toEqual({
			checked_at: "2023-11-14T22:13:20.000Z",
			latest_version: "v0.0.22",
		})
	})
...
describe("isUpdateCheckDisabled", () => {
	...
	beforeEach(() => {
		prev = process.env.VARTH_NO_UPDATE_CHECK
		// biome-ignore lint/performance/noDelete: env-var cleanup needs a real delete.
		delete process.env.VARTH_NO_UPDATE_CHECK
	})
	afterEach(() => {
		// biome-ignore lint/performance/noDelete: same as above.
		if (prev === undefined) delete process.env.VARTH_NO_UPDATE_CHECK
		else process.env.VARTH_NO_UPDATE_CHECK = prev
	})
	...
	it("returns false for empty string (matches gh CLI conventions)", () => {
		process.env.VARTH_NO_UPDATE_CHECK = ""
		expect(isUpdateCheckDisabled()).toBe(false)
	})
	it("returns true for any non-empty value", () => {
		process.env.VARTH_NO_UPDATE_CHECK = "1"
		expect(isUpdateCheckDisabled()).toBe(true)
		process.env.VARTH_NO_UPDATE_CHECK = "true"
		expect(isUpdateCheckDisabled()).toBe(true)
	})
})
