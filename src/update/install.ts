...
export function macosCodesignReSign(newPath: string): void {
	...
	if (remove.status !== 0 && (remove.error as NodeJS.ErrnoException | undefined)?.code !== "ENOENT") {
		console.error(`varth update: codesign --remove-signature failed: ${remove.stderr.trim()}`)
	}
 ...
 *   rename `varth.exe` → `varth.exe.old`, drop the new binary in as
 *   `varth.exe`, leave the .old behind for the next launch to clean up.
 *   We can't delete the old in this run because we're still it.
 *
 * Optionally backs up the old binary to ~/.cache/varth/backups/ before
...
export function atomicInstall(newPath: string, currentPath: string): { backupPath?: string } {
	...
	const backupPath = join(backupRoot, `varth.${Date.now()}`)
	try {
