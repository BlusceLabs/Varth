...
describe("isHomebrewInstall", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-paths-test-"))
	...
	it("returns false on win32 regardless of path", () => {
		setPlatform("win32")
		setExecPath("C:\\Program Files\\varth\\varth.exe")
		expect(isHomebrewInstall()).toBe(false)
	})

	it("returns true via symlink detection (bin → Cellar)", () => {
		// Fake layout:
		//   <tmp>/Cellar/varth/1.2.3/bin/varth   ← real binary
		//   <tmp>/bin/varth                        ← symlink → Cellar path
		const cellarBin = join(tmp, "Cellar", "varth", "1.2.3", "bin")
		const realBinary = join(cellarBin, "varth")
		touch(realBinary)

		const prefixBin = join(tmp, "bin")
		mkdirSync(prefixBin, { recursive: true })
		const symlink = join(prefixBin, "varth")
	...
	it("returns false when a real (non-symlink) binary sits in <prefix>/bin/", () => {
		...
		//   <tmp>/bin/varth         ← real file dropped by install.sh, NOT a symlink
		...
		const binPath = join(tmp, "bin", "varth")
		touch(binPath)
	...
	it("returns false when Cellar directory does not exist under prefix", () => {
		...
		setExecPath(join(fakePrefix, "bin", "varth"))
		expect(isHomebrewInstall()).toBe(false)
	})
})
