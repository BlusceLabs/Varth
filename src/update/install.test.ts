...
describe("copySupportingFiles", () => {
	...
	beforeEach(() => {
		srcDir = mkdtempSync(join(tmpdir(), "varth-install-src-"))
		dstDir = mkdtempSync(join(tmpdir(), "varth-install-dst-"))
	})
	...
	it("skips entries matching skipName", () => {
		writeFileSync(join(srcDir, "varth"), "binary")
		writeFileSync(join(srcDir, "keepme.txt"), "ok")

		copySupportingFiles(srcDir, dstDir, "varth")

		expect(readFileSync(join(dstDir, "keepme.txt"), "utf-8")).toBe("ok")
		expect(() => readFileSync(join(dstDir, "varth"))).toThrow()
	})
})
...
describe("atomicInstall", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-atomic-install-test-"))
	...
	it("same-fs rename: replaces current binary and creates backup on POSIX", () => {
		setPlatform("linux")
		const currentPath = join(tmp, "bin", "varth")
		const newPath = join(tmp, "new", "varth")
