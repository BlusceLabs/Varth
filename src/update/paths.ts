...
/** State file path: ~/.cache/varth/state.json (or under $XDG_CACHE_HOME). */
export function statePath(): string {
	return join(cacheDir(), APP_DIR, "state.json")
}

/** Backups dir: ~/.cache/varth/backups/. Created on demand by the installer. */
export function backupDir(): string {
	return join(cacheDir(), APP_DIR, "backups")
}

/**
 * The real path of the running varth binary, with symlinks resolved. We
 * follow symlinks so writing to the result lands on the actual file
 * (Homebrew, manual installs, etc. all symlink varth from /usr/local/bin).
 */
export function resolveExecutablePath(): string {
	// process.execPath is the running binary in a Bun-compiled `varth` —
...
/**
 * Data directory for supporting files (share/varth contents), honoring
 * XDG_DATA_HOME on Linux. Falls back to ~/.local/share/varth.
 */
