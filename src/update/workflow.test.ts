...
const REPO: Repo = { owner: "castai", name: "varth-dev", binary: "varth" }
...
describe("checkForUpdate version comparison", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-workflow-test-"))
...
describe("checkForUpdate canary path", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-workflow-canary-test-"))
		prevHome = process.env.HOME
		prevXdg = process.env.XDG_CACHE_HOME
		process.env.XDG_CACHE_HOME = tmp
	})
...
describe("applyUpdate share destination", () => {
	...
	beforeEach(() => {
		...
		fakePrefix = mkdtempSync(join(tmpdir(), "varth-prefix-test-"))
		fakeBinPath = join(fakePrefix, "bin", "varth")
		mkdirSync(join(fakePrefix, "bin"), { recursive: true })
		writeFileSync(fakeBinPath, "")

		// Create extracted archive root.
		extractRoot = mkdtempSync(join(tmpdir(), "varth-extract-test-"))
		mkdirSync(join(extractRoot, "bin"), { recursive: true })
		mkdirSync(join(extractRoot, "share", "varth"), { recursive: true })
		writeFileSync(join(extractRoot, "bin", "varth"), "")
		writeFileSync(join(extractRoot, "share", "varth", "package.json"), "{}")
	...
	it("writes share files to the sibling share when it exists", async () => {
		...
		const siblingShare = join(fakePrefix, "share", "varth")
		...
		expect(mocks.copySupportingFiles).toHaveBeenCalledWith(join(extractRoot, "share", "varth"), siblingShare, "varth")
	})

	it("falls back to ~/.local/share/varth when the sibling share does not exist", async () => {
		const client = {
			fetchChecksum: mocks.fetchChecksum,
			downloadArchive: mocks.downloadArchive,
		} as unknown as GitHubClient
		...
		expect(mocks.copySupportingFiles).toHaveBeenCalledWith(
			join(extractRoot, "share", "varth"),
			expect.stringContaining(".local/share/varth"),
			"varth",
		)
	})
})
