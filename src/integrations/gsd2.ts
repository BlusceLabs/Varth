 ...
 * billed at sticker price; everything else is free (varth-internal).
...
/**
 * Build the varth provider block for ~/.gsd/agent/models.json. GSD2 reads
 * this to populate the varth provider in its UI. All available models from
 * the API are included; cost is derived from provider (anthropic = billed,
 * rest = free) so GSD's budget tracker doesn't double-charge varth users.
 *
 ...
 */
export function buildGsd2VarthProvider(apiKey: string, models: readonly ModelMetadata[]): Record<string, unknown> {
	if (models.length === 0) {
...
async function writeGsd2(
	...
	const providers =
		existing.providers && typeof existing.providers === "object" && !Array.isArray(existing.providers)
			...
			: {}
	providers.varth = buildGsd2VarthProvider(apiKey, models)
