...
describe("buildOpenClawProviderBlock", () => {
	it("uses the varth base URL and ${VARTH_API_KEY} placeholder", () => {
		...
		expect(block.baseUrl).toBe("https://llm.varth.dev/openai/v1")
		expect(block.apiKey).toBe("${VARTH_API_KEY}")
	...
	it("includes per-model id/name/contextWindow/maxTokens", () => {
		...
		const main = block.models.find((m) => m.id === "varth/kimi-k2.6")
...
describe("buildOpenClawModelsCatalog", () => {
	it("maps each model id to its display alias", () => {
		...
		expect(catalog["varth/kimi-k2.6"]).toEqual({ alias: "Kimi K2.6" })
		expect(catalog["varth/kimi-k2.5"]).toEqual({ alias: "Kimi K2.5" })
		expect(catalog["varth/nemotron-3-ultra-fp4"]).toEqual({ alias: "Nemotron 3 Ultra FP4" })
		expect(catalog["varth/minimax-m2.7"]).toEqual({ alias: "MiniMax M2.7" })
	})
...
describe("writeOpenClawEnv", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-openclaw-test-"))
	...
	it("creates the .env file with the API key when none exists", () => {
		...
		expect(readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")).toBe("VARTH_API_KEY=test-key-123\n")
	})

	it("creates the parent directory if missing", () => {
		writeOpenClawEnv("fresh")
		expect(readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")).toBe("VARTH_API_KEY=fresh\n")
	})

	it("replaces an existing VARTH_API_KEY line in place, preserving other entries", () => {
		mkdirSync(join(tmp, ".openclaw"), { recursive: true })
		writeFileSync(join(tmp, ".openclaw", ".env"), "OTHER_VAR=keep-me\nVARTH_API_KEY=old-key\nALSO=keep\n", "utf-8")
		writeOpenClawEnv("new-key")
		expect(readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")).toBe(
			"OTHER_VAR=keep-me\nVARTH_API_KEY=new-key\nALSO=keep\n",
		)
	})

	it("appends VARTH_API_KEY when the file exists but doesn't have one yet", () => {
		mkdirSync(join(tmp, ".openclaw"), { recursive: true })
		writeFileSync(join(tmp, ".openclaw", ".env"), "OTHER=foo\n", "utf-8")
		writeOpenClawEnv("appended")
		expect(readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")).toBe("OTHER=foo\nVARTH_API_KEY=appended\n")
	})
})
...
describe("writeOpenClawDirect integration", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-openclaw-direct-"))
		prevHome = process.env.HOME
		process.env.HOME = tmp
	...
	it("preserves existing user config keys like temperature and fallbacks", async () => {
		...
		expect(written.agents.defaults.model.fallbacks).toContain("varth/nemotron-3-ultra-fp4")
		expect(written.agents.defaults.model.fallbacks).toContain("varth/minimax-m2.7")
		expect(written.agents.defaults.models["other/model"]).toEqual({ alias: "Other" })
		expect(written.agents.defaults.models["varth/kimi-k2.6"]).toBeDefined()
		expect(written.models.providers.varth).toBeDefined()
	})
})
...
describe("writeOpenClawViaCLI integration", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-openclaw-cli-"))
		prevHome = process.env.HOME
		process.env.HOME = tmp
	...
	it("preserves existing fallbacks and other keys via CLI merge", async () => {
		...
		expect(mergedFallbacks).toContain("varth/nemotron-3-ultra-fp4")
		...
		expect(mergedModels["varth/kimi-k2.6"]).toBeDefined()
		expect(mergedModels["varth/nemotron-3-ultra-fp4"]).toBeDefined()

		const envContent = readFileSync(join(tmp, ".openclaw", ".env"), "utf-8")
		expect(envContent).toBe("VARTH_API_KEY=test-key-123\n")
	})
})
