...
describe("claudeCodeEnv", () => {
	it("emits the four env vars Claude Code expects, with ANTHROPIC_API_KEY explicitly empty", () => {
		...
		expect(env).toEqual({
			ANTHROPIC_BASE_URL: "https://llm.varth.dev/anthropic",
			ANTHROPIC_API_KEY: "",
...
describe("claude-code tool registration", () => {
	...
	beforeEach(async () => {
		scratchHome = mkdtempSync(join(tmpdir(), "varth-claude-test-"))
	...
	it("write() merges env into ~/.claude/settings.json without clobbering other keys", async () => {
		...
		expect(written.env.ANTHROPIC_BASE_URL).toBe("https://llm.varth.dev/anthropic")
	...
	it("write() rejects when ANTHROPIC_AUTH_TOKEN is missing after write", async () => {
		...
		vi.mocked(readJson)
			...
			.mockReturnValueOnce({ env: { ANTHROPIC_BASE_URL: "https://llm.varth.dev/anthropic" } })
