...
describe("extractVersionFromPluginName", () => {
	it("returns the suffix after @", () => {
		expect(extractVersionFromPluginName("@varth-dev/opencode-varth@1.14.0")).toBe("1.14.0")
		expect(extractVersionFromPluginName("@varth-dev/opencode-varth@latest")).toBe("latest")
	})
	it("returns null for the bare package name (no version pin)", () => {
		expect(extractVersionFromPluginName("@varth-dev/opencode-varth")).toBeNull()
	})
...
describe("buildUpdatedPlugins", () => {
	const VARTH = "@varth-dev/opencode-varth"

	it("inserts a Varth entry when none exists, using latestVersion", () => {
		const r = buildUpdatedPlugins({ plugins: [], telemetryEnabled: false, latestVersion: "1.14.0" })
		expect(r.skippedVarthPlugin).toBe(false)
		expect(r.plugins).toEqual([[`${VARTH}@1.14.0`, {}]])
	})
	it("preserves unrelated plugins and replaces only the Varth entry", () => {
		const existing = [
			["@other/plugin@1.0.0", { foo: "bar" }],
			[`${VARTH}@1.10.0`, { stale: true }],
		]
	...
	it("falls back to the existing Varth version when npm is unavailable", () => {
		const existing = [[`${VARTH}@1.13.0`, {}]]
		const r = buildUpdatedPlugins({ plugins: existing, telemetryEnabled: false, latestVersion: null })
		expect(r.plugins).toEqual([[`${VARTH}@1.13.0`, {}]])
		expect(r.skippedVarthPlugin).toBe(false)
	})
	it("skips the Varth plugin when there's no version anywhere (npm + no prior entry)", () => {
		...
		expect(r.skippedVarthPlugin).toBe(true)
	})
	...
	it("strips the bare-package form (no version pin) like the versioned form", () => {
		const existing = [[VARTH, { stale: true }]]
	...
	it("handles legacy string format (no version pin) and re-adds with pinned version", () => {
		const existing = ["@other/plugin", VARTH]
		const r = buildUpdatedPlugins({ plugins: existing, telemetryEnabled: false, latestVersion: "1.14.0" })
		expect(r.plugins).toEqual(["@other/plugin", [`${VARTH}@1.14.0`, {}]])
		expect(r.skippedVarthPlugin).toBe(false)
	})
	it("handles legacy string format with existing version and falls back to it when npm unavailable", () => {
		const existing = ["@other/plugin", `${VARTH}@1.13.0`]
		const r = buildUpdatedPlugins({ plugins: existing, telemetryEnabled: false, latestVersion: null })
		expect(r.plugins).toEqual(["@other/plugin", [`${VARTH}@1.13.0`, {}]])
		expect(r.skippedVarthPlugin).toBe(false)
	})
	it("skips legacy string Varth plugin when no version anywhere (npm + no prior entry)", () => {
		const r = buildUpdatedPlugins({ plugins: ["@other/plugin", VARTH], telemetryEnabled: false, latestVersion: null })
...
describe("opencode tool registration", () => {
	...
	beforeEach(() => {
		scratchHome = mkdtempSync(join(tmpdir(), "varth-opencode-test-"))
	...
	it("write() merges provider + plugin into opencode.json without clobbering unrelated keys", async () => {
		...
		try {
			...
			expect(written.provider.varth).toBeDefined()
			expect((written.provider.varth as { options: { apiKey: string } }).options.apiKey).toBe("test-key-123")
			expect(written.model).toBe("varth/kimi-k2.6")
