...
describe("mergeCursorConfig", () => {
	it("sets the base URL and useOpenAIKey on a fresh blob", () => {
		...
		expect(out.openAIBaseUrl).toBe("https://llm.varth.dev/openai/v1")
		expect(out.useOpenAIKey).toBe(true)
	})

	it("populates aiSettings.modelConfig with the varth-routed surfaces", () => {
		...
		expect(mc.composer.modelName).toBe("varth/kimi-k2.6")
		expect(mc.spec.modelName).toBe("varth/kimi-k2.6")
		expect(mc["deep-search"].modelName).toBe("varth/kimi-k2.6")
		expect(mc["background-composer"].modelName).toBe("varth/kimi-k2.6")
		expect(mc["composer-ensemble"].modelName).toBe("varth/kimi-k2.6")
		expect(mc["cmd-k"].modelName).toBe("varth/nemotron-3-ultra-fp4")
		expect(mc["plan-execution"].modelName).toBe("varth/nemotron-3-ultra-fp4")
		expect(mc["quick-agent"].modelName).toBe("varth/minimax-m2.7")
	})

	it("appends ALL model slugs to userAddedModels without duplicating existing ones", () => {
		const initial = {
			aiSettings: {
				userAddedModels: ["other/model-a", "varth/kimi-k2.6"],
			},
		}
		...
		// is preserved, "varth/kimi-k2.6" isn't duplicated. Anthropic model
