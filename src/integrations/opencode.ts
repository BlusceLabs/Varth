 ...
 * `["@varth-dev/opencode-varth"]` string list which loses the per-plugin
 * config block.
 */
 ...
 * `@varth-dev/opencode-varth@1.14.0` → `"1.14.0"`. Returns null for the
 * un-pinned form (`@varth-dev/opencode-varth`).
 */
...
interface PluginUpdateResult {
	...
	skippedVarthPlugin: boolean
}

/**
 * Pure transform on the plugin array: strip out any prior Varth entries,
...
export function buildUpdatedPlugins(inputs: PluginUpdateInputs): PluginUpdateResult {
	...
	for (const entry of plugins) {
		if (Array.isArray(entry) && entry.length > 0 && typeof entry[0] === "string") {
			...
			// Legacy string format: "@varth-dev/opencode-varth" or "@varth-dev/opencode-varth@1.14.0"
	...
	if (!versionToPin) {
		return { plugins: filtered, skippedVarthPlugin: true }
	}

	const updatedEntry: unknown[] = [`${OPENCODE_PLUGIN_PACKAGE}@${versionToPin}`, pluginConfig]
	return { plugins: [...filtered, updatedEntry], skippedVarthPlugin: false }
}
