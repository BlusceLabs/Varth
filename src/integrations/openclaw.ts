 ...
 * `models.providers.varth`. Same shape whether we write it via the CLI
 * batch flag or directly into ~/.openclaw/openclaw.json. Pure so we can
 * snapshot-test it without exec or fs.
 *
 * `apiKey` deliberately points at `${VARTH_API_KEY}` rather than the raw
...
/**
 * Write `VARTH_API_KEY=<key>` into ~/.openclaw/.env, replacing any prior
 * line for the same key. The .env file feeds the OpenClaw daemon, which
 * interpolates `${VARTH_API_KEY}` from openclaw.json at runtime.
 */
...
async function writeOpenClawViaCLI(
	...
	runOpenClawCmd(["config", "set", "models.providers.varth", JSON.stringify(providerBlock)])
	...
	if (isOpenClawGatewayRunning()) {
		...
		// varth provider; choosing custom-api-key would create a duplicate
