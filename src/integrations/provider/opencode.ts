...
/**
 * Build the OpenCode provider config block for the varth provider.
 * OpenCode pickers fail silently on unknown keys, so keep the schema
 * exact.
 *
 * The shape is `existing.provider.varth = …` in opencode.json. All
...
function openCodeModelEntry(model: ModelMetadata): Record<string, unknown> {
	return {
		name: model.slug,
		// `tool_call` is not in ModelMetadata; all models served through varth support it.
		tool_call: true,
