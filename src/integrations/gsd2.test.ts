...
import { buildGsd2Preferences, buildGsd2VarthProvider } from "./gsd2.js"
import { byId } from "./registry.js"

describe("buildGsd2VarthProvider", () => {
	it("returns the provider block with apiKey, baseUrl, and defaultModel", () => {
		const p = buildGsd2VarthProvider("test-key", TEST_MODELS) as {
			apiKey: string
			baseUrl: string
			defaultModel: string
		}
		expect(p.apiKey).toBe("test-key")
		expect(p.baseUrl).toBe("https://llm.varth.dev/openai/v1")
		// defaultModel is the resolved main model (first serverless vision model).
		expect(p.defaultModel).toBe("kimi-k2.6")
	})

	it("emits all models with cost metadata (Opus/Sonnet billed at sticker, varth models free)", () => {
		const p = buildGsd2VarthProvider("k", TEST_MODELS) as {
	...
	it("Kimi models accept text+image input, others text only", () => {
		const p = buildGsd2VarthProvider("k", TEST_MODELS) as { models: Array<{ id: string; input: string[] }> }
...
describe("buildGsd2Preferences", () => {
	...
	it("routes the high-level task buckets to the right model tiers", () => {
		...
		expect(prefs).toMatch(/planning: varth\/(claude-opus-4-6|kimi-k2\.6)/)
		expect(prefs).toMatch(/validation: varth\/(claude-opus-4-6|kimi-k2\.6)/)
		expect(prefs).toMatch(/auto_supervisor:\s*\n\s*model: varth\/(claude-opus-4-6|kimi-k2\.6)/)
		// Coding model for research + subagent.
		expect(prefs).toContain("research: varth/nemotron-3-ultra-fp4")
		expect(prefs).toContain("subagent: varth/nemotron-3-ultra-fp4")
		// Sub model for execution_simple + discuss.
		expect(prefs).toContain("discuss: varth/minimax-m2.7")
		expect(prefs).toContain("execution_simple: varth/minimax-m2.7")
	})

	it("uses the new tier_models shape (light → coding, standard → sub, heavy → [main, planning])", () => {
		expect(prefs).toContain("light: varth/nemotron-3-ultra-fp4")
		expect(prefs).toContain("standard: varth/minimax-m2.7")
		expect(prefs).toMatch(/heavy:\s*\n\s*- varth\/kimi-k2\.6\s*\n\s*- varth\/(claude-opus-4-6|kimi-k2\.6)/)
	})
...
describe("gsd2 tool registration", () => {
	...
	beforeEach(() => {
		tmp = mkdtempSync(join(tmpdir(), "varth-gsd2-test-"))
	...
	it("write() emits both ~/.gsd/agent/models.json and ~/.gsd/preferences.md", async () => {
		...
		expect(models.providers.varth.apiKey).toBe("secret-123")
	...
	it("write() preserves other providers and top-level keys in models.json", async () => {
		...
		writeFileSync(
			...
			JSON.stringify({
				...
				providers: {
					...
					varth: { apiKey: "stale-varth-key", legacy: true },
				},
			}),
		)

		const tool = byId("gsd2")
		await tool?.write("global", "fresh-varth-key", TEST_MODELS)
		...
		expect(models.providers.varth.apiKey).toBe("fresh-varth-key")
		expect(models.providers.varth.legacy).toBeUndefined()
	})
})
