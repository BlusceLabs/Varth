...
describe("loadConfig", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "varth-test-"))
	...
	it("reads project config from .varth/config.json overriding global", () => {
		const globalDir = mkdtempSync(join(tmpdir(), "varth-test-"))
		const projectDir = mkdtempSync(join(tmpdir(), "varth-test-"))
		const globalPath = join(globalDir, "config.json")
		const projectPath = join(projectDir, ".varth", "config.json")
	...
	it("falls back to global when .varth/config.json does not exist", () => {
		const globalDir = mkdtempSync(join(tmpdir(), "varth-test-"))
		const projectDir = mkdtempSync(join(tmpdir(), "varth-test-"))
		const globalPath = join(globalDir, "config.json")
	...
	it("does not walk up the directory tree from a subfolder", () => {
		const globalDir = mkdtempSync(join(tmpdir(), "varth-test-"))
		const rootDir = mkdtempSync(join(tmpdir(), "varth-test-"))
		const subDir = join(rootDir, "src", "components")
		const globalPath = join(globalDir, "config.json")
		const rootProjectPath = join(rootDir, ".varth", "config.json")
		...
		// cwd is a subfolder that does NOT have .varth/config.json
		const config = loadConfig({ configPath: globalPath, cwd: subDir })
		// Should NOT pick up rootDir/.varth/config.json
...
describe("writeApiKey", () => {
	...
	it("can persist the Varth API endpoint with the API key", () => {
		writeApiKey("sekrit-42", configPath, { llmEndpoint: " https://custom.varth.example " })
		const raw = JSON.parse(readFileSync(configPath, "utf-8"))
		expect(raw.apiKey).toBe("sekrit-42")
		expect(raw.llmEndpoint).toBe("https://custom.varth.example")
	})
...
describe("writeDeviceId", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "varth-test-device-"))
		configPath = join(tempDir, "config.json")
	})
...
describe("readTelemetryConfig", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "varth-telemetry-test-"))
		configPath = join(tempDir, "config.json")
		savedApiKey = process.env.VARTH_API_KEY
		savedTelemetryEnabled = process.env.VARTH_TELEMETRY_ENABLED
		// biome-ignore lint/performance/noDelete: env var must be deleted, not set to "undefined"
		delete process.env.VARTH_API_KEY
		// biome-ignore lint/performance/noDelete: env var must be deleted, not set to "undefined"
		delete process.env.VARTH_TELEMETRY_ENABLED
	})

	afterEach(() => {
		rmSync(tempDir, { recursive: true, force: true })
		if (savedApiKey !== undefined) process.env.VARTH_API_KEY = savedApiKey
		// biome-ignore lint/performance/noDelete: env var must be deleted, not set to "undefined"
		else delete process.env.VARTH_API_KEY
		if (savedTelemetryEnabled !== undefined) process.env.VARTH_TELEMETRY_ENABLED = savedTelemetryEnabled
		// biome-ignore lint/performance/noDelete: env var must be deleted, not set to "undefined"
		else delete process.env.VARTH_TELEMETRY_ENABLED
	})
	...
	it("injects User-Agent into telemetry headers", () => {
		...
		expect(config.headers["User-Agent"]).toMatch(/^varth\//)
	})
...
describe("getActiveVendorSkillPaths", () => {
	...
	beforeEach(() => {
		...
		mockHome = mkdtempSync(join(tmpdir(), "varth-vendor-test-"))
	...
	it("returns vendor paths when vendor dir exists and no sentinel", () => {
		mkdirSync(join(mockHome, ".config", "varth", "vendor", "superpowers", "skills"), { recursive: true })
	...
	it("returns empty array when sentinel exists in harness skills dir", () => {
		mkdirSync(join(mockHome, ".config", "varth", "harness", "skills", "using-superpowers"), { recursive: true })
		writeFileSync(join(mockHome, ".config", "varth", "harness", "skills", "using-superpowers", "SKILL.md"), "")
...
describe("buildSkillPathOptions", () => {
	...
	beforeEach(() => {
		...
		tempDir = mkdtempSync(join(tmpdir(), "varth-skillpaths-"))
	})
