...
function creds(overrides: Partial<WorkspaceCredentials> = {}): WorkspaceCredentials {
	return {
		wsUrl: "wss://ws-1.remote.varth.dev",
		host: "ws-1.remote.varth.dev",
...
describe("deriveBaseUrl", () => {
	it("converts wss:// → https://", () => {
		expect(deriveBaseUrl("wss://ws-1.remote.varth.dev")).toBe("https://ws-1.remote.varth.dev")
	})
...
describe("WorkerClient", () => {
	...
	it("attaches Bearer token to GET", async () => {
		...
		expect(mockFetch.mock.calls[0][0]).toBe("https://ws-1.remote.varth.dev/status")
