...
export function resolveEndpoint(options?: AuthenticateOptions): string {
	const fromEnv = process.env.VARTH_REMOTE_ENDPOINT
	return options?.endpoint ?? fromEnv ?? "https://app.varth.dev/api"
}
...
export async function checkResponse(resp: Response, endpoint: string): Promise<void> {
	...
	switch (resp.status) {
		case 401:
			throw new RemoteAuthError(`Invalid API key - run 'varth setup' to authenticate: ${endpoint}`, 401)
		case 403:
