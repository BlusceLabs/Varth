...
describe("authenticateWorkspace", () => {
	it("returns WorkspaceCredentials after the 3-step flow with correct URLs and methods", async () => {
			...
			.mockResolvedValueOnce(
				new Response(
					JSON.stringify({
						...
						status: "INITIALIZING",
						uri: "wss://s-3380b7aa.remote.varth.dev",
					}),
		...
		expect(result.wsUrl).toBe("wss://s-3380b7aa.remote.varth.dev")
		expect(result.host).toBe("s-3380b7aa.remote.varth.dev")
	...
	it("normalizes a bare-hostname URI returned by the server", async () => {
		const bare = "trusting-titan.remote.varth.dev"
