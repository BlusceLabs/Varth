...
const VARTH_CONFIG_PATH = resolve(homedir(), ".config", "varth", "config.json")
const AGENT_CONFIG_DIR = resolve(homedir(), ".config", "varth", "harness")
...
export const ALWAYS_SHOWN_SKILL_PATHS = [join(".config", "varth", "harness", "skills")]
...
export function getActiveVendorSkillPaths(): string[] {
	const home = homedir()
	const harnessSkillsDir = join(home, ".config", "varth", "harness", "skills")
...
export const envConfig = {
	VARTH_WEB_APP_URL: process.env.VARTH_WEB_APP_URL ?? "https://app.varth.dev",
}
...
/**
 * Read the Cast AI API key from the varth CLI config file.
 * Returns undefined if the file doesn't exist or the field is missing.
 */
export function readApiKeyFromConfigFile(configPath: string = VARTH_CONFIG_PATH): string | undefined {
	try {
...
export function readTelemetryConfig(configPath?: string): TelemetryConfig {
	...
	const apiKey =
		(typeof process.env.VARTH_API_KEY === "string" && process.env.VARTH_API_KEY.length > 0
			? process.env.VARTH_API_KEY
...
/**
 * Load the varth configuration.
 *
 * Config precedence (highest to lowest):
 *   1. VARTH_API_KEY environment variable (highest precedence)
 *   2. Project .varth/config.json (if cwd provided)
 *   3. Global ~/.config/varth/config.json
 *
 ...
 */
export function loadConfig(options?: { configPath?: string; cwd?: string }): VarthConfig {
	// Read global config
	const globalConfigPath = options?.configPath ?? VARTH_CONFIG_PATH
	const globalExtras = readConfigExtras(globalConfigPath)

	// Read project-level config
	const projectPath = resolve(options?.cwd ?? process.cwd(), ".varth", "config.json")
...
export function readSessionModeWizardSeenAt(configPath?: string): string | undefined {
	return readConfigExtras(configPath ?? VARTH_CONFIG_PATH).onboarding?.sessionModeWizardSeenAt
}
...
export function readTeleportHelpSeenAt(configPath?: string): string | undefined {
	return readConfigExtras(configPath ?? VARTH_CONFIG_PATH).onboarding?.teleportHelpSeenAt
}
...
export function readSurveySeenAt(surveyId: string, configPath?: string): string | undefined {
	return readSurveyConfig(surveyId, configPath ?? VARTH_CONFIG_PATH)?.seenAt
}

export function writeSurveySeenAt(surveyId: string, seenAt: string, configPath?: string): void {
	updateSurveyConfig(configPath ?? VARTH_CONFIG_PATH, surveyId, (survey) => {
		survey.seenAt = seenAt
	})
}

export function readHideSessionModeDialog(configPath?: string): boolean {
	return readConfigExtras(configPath ?? VARTH_CONFIG_PATH).onboarding?.hideSessionModeDialog === true
}

export function readHideTips(configPath?: string): boolean {
	return readConfigExtras(configPath ?? VARTH_CONFIG_PATH).preferences?.hideTips === true
}
...
export function writeMigrationState(state: MigrationState, configPath?: string): void {
	writeConfigField("migrationState", state, configPath ?? VARTH_CONFIG_PATH)
}

export function writeSkillPaths(paths: string[], configPath?: string): void {
	writeConfigField("skillPaths", paths, configPath ?? VARTH_CONFIG_PATH)
}
...
/**
 * Persist telemetry.enabled in the varth config.json. Used by
 * `varth config telemetry on|off`. The reader (readTelemetryConfig) already
 * honours an env-var override, so this is a no-op for sessions where
 * VARTH_TELEMETRY_ENABLED is set — but the persisted value is still useful
 * for fresh shells.
 */
/**
 * Check whether the user has explicitly set a telemetry preference —
 * either via the VARTH_TELEMETRY_ENABLED env var or by persisting
...
export function isTelemetryExplicitlyConfigured(configPath?: string): boolean {
	if (process.env.VARTH_TELEMETRY_ENABLED !== undefined) return true
	try {
		const path = configPath ?? VARTH_CONFIG_PATH
		const raw = readFileSync(path, "utf-8")
		const parsed = JSON.parse(raw)
 ...
 * security model (plaintext in `~/.config/varth/config.json`).
 */
