 ...
 * |---|---|---|---|---|
 * | adhoc (`--plan`) | approved plan text | `.md` | `<cwd>/.varth/plans/plan-<timestamp>.md` | `saveApprovedPlan` (`permissions/plan-persistence.ts:4-11`) |
 * | ferment | structured ferment artifact | `.json` | `<cwd>/.varth/ferments/<id>.json` | inline at `permissions/index.ts:602-642` (pre-Phase-4 placeholder) |
 *
 * ## Why two stores?
 *
...
// ---------------------------------------------------------------------------
// AdhocPlanStore — `.varth/plans/<timestamp>.md`
// ---------------------------------------------------------------------------

const ADHOC_DIR = ".varth/plans"
const ADHOC_PREFIX = "plan-"

/**
 * Persists approved `--plan` mode artifacts as Markdown in
 * `<cwd>/.varth/plans/plan-<timestamp>.md`. Mirrors the existing
...
// ---------------------------------------------------------------------------
// FermentPlanStore — `.varth/ferments/<id>.json`
// ---------------------------------------------------------------------------

const FERMENT_DIR = ".varth/ferments"
const FERMENT_EXT = ".json"

/**
 * Persists ferment artifacts as JSON in `<cwd>/.varth/ferments/<id>.json`.
