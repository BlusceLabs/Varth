...
beforeEach(() => {
	process.env.VARTH_CODING_AGENT_DIR = "/fake/agent/dir"
	vi.useFakeTimers()
})
...
describe("getActiveThemeName", () => {
	...
	it("returns undefined when VARTH_CODING_AGENT_DIR is unset", () => {
		// biome-ignore lint/performance/noDelete: process.env requires delete operator to be truly unset rather than stringified to "undefined"
		delete process.env.VARTH_CODING_AGENT_DIR
		expect(getActiveThemeName()).toBeUndefined()
	})
...
describe("onThemeChange", () => {
	it("does not fire listener when theme has not changed", () => {
		mockReadFileSync.mockReturnValue(JSON.stringify({ theme: "varth-minimal" }))
		const listener = vi.fn()

		// Subscribe — watcher captures lastSeenTheme = "varth-minimal"
	...
	it("fires listener when theme changes", () => {
		mockReadFileSync
			.mockReturnValueOnce(JSON.stringify({ theme: "varth-minimal" })) // ensureWatcher init read
		...
		expect(listener).toHaveBeenCalledWith("dark", "varth-minimal")
		unsub()
	})
