...
// Semantic state foregrounds, matching master's varth.json hexes (teal-500 /
// amber-500 / red-500). Used by surfaces like the permissions footer that need
// fixed semantic colors regardless of the active theme — the varth-minimal
// theme intentionally inherits these tokens from the terminal scheme, which
// flattens the visual cue, so callers hardcode the varth values here.
...
// Returns the theme's ANSI open-sequence for `c`, or the hardcoded varth
// palette value when the theme has an empty token (e.g. varth-minimal).
...
// varth-minimal which intentionally leaves accent empty to inherit from
// the terminal scheme.
