...
// @agentclientprotocol/sdk. Lets IDE extensions, Zed, openclaw drive varth in-process.
...
export class VarthAcpAgent implements Agent {
	 ...
	 * 1. Environment variable VARTH_PERMISSIONS
	...
	async newSession(params: NewSessionRequest): Promise<NewSessionResponse> {
		// mcpServers isn't plumbed: varth loads MCP servers from its own config via
		...
		if (Array.isArray(params.mcpServers) && params.mcpServers.length > 0) {
			throw RequestError.invalidParams(undefined, "mcpServers is not supported; configure MCP servers via varth config")
		}
...
// VARTH_API_KEY before we ever spawned the ACP loop, and updateModelsConfig
...
// listSessions points at varth's agentDir (~/.config/varth/harness/sessions/...)
// instead of pi's own ~/.pi/agent/sessions/... — pi reads PI_CODING_AGENT_DIR,
// not VARTH_CODING_AGENT_DIR, so without explicit sessionDir threading the
// default lookup misses every varth session. Encoding is a public on-disk
...
function defaultSessionLoader(options: RunAcpOptions): AcpSessionLoader {
	return async (params: LoadSessionRequest): Promise<AgentSession> => {
		...
		// not VARTH_CODING_AGENT_DIR, so default lookups would miss varth
		// sessions stored under the varth agent dir.
...
// Mirrors the tool names varth actually exposes: pi-coding-agent core tools
// plus the varth extensions in src/extensions (web-fetch, web-search, Agent).
...
export async function runAcpMode(options: RunAcpOptions): Promise<void> {
	...
	let agentInstance: VarthAcpAgent | undefined
	const conn = new AgentSideConnection((c) => {
		agentInstance = new VarthAcpAgent(c, options)
