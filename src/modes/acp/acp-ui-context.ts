	...
	const ui: ExtensionUIContext = {
		...
		async editor(title, prefill) {
			...
			if (!supportsMethod("pi_editor")) {
				warnUnsupportedMethod(
					"editor",
					`Extension requested an editor ("${title}") but the client doesn't advertise the _varth.dev/pi_editor capability. The request was dropped.`,
				)
				return undefined
			}
		...
		notify(message, type) {
			if (!supportsMethod("pi_notify")) {
				warnUnsupportedMethod(
					"notify",
					`Extension notification dropped (client doesn't advertise _varth.dev/pi_notify): ${message}`,
				)
				return undefined
			}
		...
		setStatus(key, text) {
			if (!supportsMethod("pi_setStatus")) {
				warnUnsupportedMethod(
					"setStatus",
					`Extension tried to set status "${key}" but the client doesn't advertise _varth.dev/pi_setStatus. The update was dropped.`,
				)
				return
			}
			notify("pi_setStatus", {
		...
		setWidget: (key, content, options) => {
			...
			if (!supportsMethod("pi_setWidget")) {
				warnUnsupportedMethod(
					"setWidget",
					`Extension tried to set widget "${key}" but the client doesn't advertise _varth.dev/pi_setWidget. The widget was dropped.`,
				)
				return
			}
		...
		setEditorText(text) {
			if (!supportsMethod("pi_set_editor_text")) {
				warnUnsupportedMethod(
					"setEditorText",
					`Extension tried to set editor text but the client doesn't advertise _varth.dev/pi_set_editor_text. The update was dropped.`,
				)
				return
			}
