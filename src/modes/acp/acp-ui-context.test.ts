...
describe("createAcpUIContext — input via elicitation", () => {
	...
	it("falls back to notify + undefined when client lacks form elicitation (no permission equivalent for free text)", async () => {
		...
		expect(method).toBe("_varth.dev/pi_notify")
		expect(params).toEqual({
...
describe("createAcpUIContext — capability gating", () => {
	...
	it("prefers elicitation when both elicitation and _varth.dev/pi are advertised", async () => {
...
describe("createAcpUIContext — editor (still on _varth.dev/pi namespace)", () => {
	...
	it("calls _varth.dev/pi_editor and returns the edited text", async () => {
		...
		expect(method).toBe("_varth.dev/pi_editor")
		expect(params).toEqual({
			id: expect.any(String),
			type: "extension_ui_request",
			method: "editor",
	...
	it("resolves undefined and warns via agent_message_chunk when the client doesn't advertise the pi capability", async () => {
		...
		expect(params).toEqual({
			...
			update: {
				content: {
					text: '[ACP] editor: Extension requested an editor ("Edit") but the client doesn\'t advertise the _varth.dev/pi_editor capability. The request was dropped.',
					type: "text",
				},
				sessionUpdate: "agent_message_chunk",
			},
...
describe("createAcpUIContext — fire-and-forget notifications", () => {
	...
	it("notify sends _varth.dev/pi_notify via extNotification with notifyType undefined when type is omitted", async () => {
		const {
			conn,
			extNotification: n,
			send,
	...
	it("setStatus sends _varth.dev/pi_setStatus via extNotification", async () => {
		const {
			conn,
			extNotification: n,
			send,
		...
		expect(n).toHaveBeenCalledTimes(1)
		const [method, params] = n.mock.calls[0]
		expect(method).toBe("_varth.dev/pi_setStatus")
		expect(params).toEqual({
			id: expect.any(String),
			type: "extension_ui_request",
			method: "setStatus",
	...
	it("setStatus warns via agent_message_chunk when the client doesn't advertise support", () => {
		...
		expect(params).toEqual({
			...
			update: {
				content: {
					text: '[ACP] setStatus: Extension tried to set status "k" but the client doesn\'t advertise _varth.dev/pi_setStatus. The update was dropped.',
					type: "text",
				},
				sessionUpdate: "agent_message_chunk",
			},
	...
	it("setEditorText sends _varth.dev/pi_set_editor_text via extNotification", async () => {
		const {
			conn,
			extNotification: n,
			send,
		...
		expect(n).toHaveBeenCalledTimes(1)
		const [method, params] = n.mock.calls[0]
		expect(method).toBe("_varth.dev/pi_set_editor_text")
		expect(params).toEqual({
			id: expect.any(String),
			type: "extension_ui_request",
	...
	it("notify warns via agent_message_chunk and includes the message text", () => {
		...
		expect(params).toEqual({
			...
			update: {
				content: {
					text: "[ACP] notify: Extension notification dropped (client doesn't advertise _varth.dev/pi_notify): first try",
					type: "text",
				},
				sessionUpdate: "agent_message_chunk",
			},
	...
	it("setWidget (string[] branch) forwards to _varth.dev/pi_setWidget", async () => {
		const {
			conn,
			extNotification: n,
			send,
		...
		expect(n).toHaveBeenCalledTimes(1)
		const [method, params] = n.mock.calls[0]
		expect(method).toBe("_varth.dev/pi_setWidget")
		expect(params).toEqual({
			id: expect.any(String),
			type: "extension_ui_request",
			method: "setWidget",
	...
	it("setWidget warns via agent_message_chunk when the client doesn't advertise support", () => {
		...
		expect(params).toEqual({
			...
			update: {
				content: {
					text: '[ACP] setWidget: Extension tried to set widget "todo" but the client doesn\'t advertise _varth.dev/pi_setWidget. The widget was dropped.',
					type: "text",
				},
				sessionUpdate: "agent_message_chunk",
			},
