...
// Minimal fake of AgentSession surface used by VarthAcpAgent. The factory seam
...
describe("VarthAcpAgent turn lifecycle", () => {
	...
	describe("initialize capability detection", () => {
		const tempAgentDir = "/tmp/varth-acp-test-agent-dir"
		...
		it("declares image: true when cached models include vision models", async () => {
			...
			try {
				...
				const testAgent = new VarthAcpAgent(makeConn(), {
					extensionFactories: [],
	...
	it("drops stray tool_execution_start/end after a short-circuited turn", async () => {
		...
		const localAgent = new VarthAcpAgent(conn, {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
	...
	it("disposes the session if subscribe throws during newSession", async () => {
		...
		const localAgent = new VarthAcpAgent(makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory: factory,
		})
	...
	// mcpServers is declared in the ACP request shape but varth has no hook to
	...
	it("isolates turn state across parallel sessions", async () => {
		...
		const parallelAgent = new VarthAcpAgent(makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
...
describe("VarthAcpAgent tool execution stream", () => {
	let fake: FakeAgentSession
	let agent: VarthAcpAgent
	let sessionId: string
	...
	beforeEach(async () => {
		...
		updates = rec.updates
		agent = new VarthAcpAgent(rec.conn, {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory,
		})
...
describe("newSession model state", () => {
	it("returns model state in the response when a model is available", async () => {
		...
		const agent = new VarthAcpAgent(makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory: factory,
		})
...
describe("newSession available commands", () => {
	it("sends available_commands_update with /bug command on session init", async () => {
		...
		const factory: AcpSessionFactory = async () => asSession(fake)
		const { conn, updates } = makeRecordingConn()
		const agent = new VarthAcpAgent(conn, {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory: factory,
		})
...
describe("session mode controller lifecycle", () => {
	...
	it("defaults to 'default' mode when VARTH_PERMISSIONS env is unset", async () => {
		vi.stubEnv(PERMISSIONS_ENV_KEY, "")
		Reflect.deleteProperty(process.env, PERMISSIONS_ENV_KEY)
		const agent = new VarthAcpAgent(makeConn(), {
			extensionFactories: [],
	...
	it("uses defaultMode from permissions config file when env is not set", async () => {
		// Create a temp directory with a .varth/permissions.json
		const tmpDir = mkdtempSync(join(tmpdir(), "acp-mode-config-test-"))
		const varthDir = join(tmpDir, ".varth")
		mkdirSync(varthDir, { recursive: true })
		writeFileSync(join(varthDir, "permissions.json"), JSON.stringify({ defaultMode: "plan" }))
	...
	it("env VARTH_PERMISSIONS takes precedence over config defaultMode", async () => {
		// Create a temp directory with a .varth/permissions.json
	...
	it("closeSession deletes the VARTH_PERMISSIONS_<sessionId> env key", async () => {
		vi.stubEnv(PERMISSIONS_ENV_KEY, "")
		Reflect.deleteProperty(process.env, PERMISSIONS_ENV_KEY)
	...
	it("shutdown deletes VARTH_PERMISSIONS_<sessionId> env keys for all sessions", async () => {
		vi.stubEnv(PERMISSIONS_ENV_KEY, "")
		Reflect.deleteProperty(process.env, PERMISSIONS_ENV_KEY)
...
describe("VarthAcpAgent listSessions", () => {
	function makeAgent(lister: AcpSessionLister): VarthAcpAgent {
		return new VarthAcpAgent(makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory: async () => asSession(new FakeAgentSession("unused")),
...
describe("VarthAcpAgent loadSession", () => {
	function makeAgent(loader: AcpSessionLoader, opts?: { conn?: AgentSideConnection }): VarthAcpAgent {
		return new VarthAcpAgent(opts?.conn ?? makeConn(), {
			extensionFactories: [],
			agentDir: "/tmp/fake-agent-dir",
			sessionFactory: async () => asSession(new FakeAgentSession("unused")),
			sessionLoader: loader,
	...
	it("returns configOptions in loadSession response", async () => {
		const tmpDir = mkdtempSync(join(tmpdir(), "varth-acp-load-config-"))
		try {
	...
	it("rejects default-loaded sessions whose header cwd disagrees before opening", async () => {
		const agentDir = mkdtempSync(join(tmpdir(), "varth-acp-load-"))
		try {
