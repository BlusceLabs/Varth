...
A coding agent CLI powered by [varth](https://varth.dev/). Built on the [pi-mono](https://github.com/badlogic/pi-mono) coding agent SDK, varth gives you an AI-powered development assistant in your terminal that connects to varth's LLM infrastructure.
...
```bash
brew install getvarth/tap/varth
```

**Install script:**

```bash
curl -fsSL https://BlusceLabs/varth/releases/latest/download/install.sh | bash
```

Then configure your API key and launch:

```bash
varth setup   # one-time interactive setup
varth         # launch the coding agent
```

Run `varth --help` to see all available subcommands and flags.

## Models

### Model selection

The supported model list is fetched at startup from the varth metadata service. Use `/model` or `ctrl+p` in the interactive CLI to switch between available models.

Varth operates in one of two modes:
...
Use `/multi-model` in the interactive CLI to toggle models on/off per role, or edit `~/.config/varth/harness/settings.json` directly:

```json
{
  "modelRoles": {
    "orchestrator": "varth-dev/kimi-k2.6",
    "builder": ["varth-dev/minimax-m2.7", "anthropic/claude-sonnet-4-5"],
    "reviewer": "anthropic/claude-sonnet-4-5",
    "explorer": "varth-dev/nemotron-3-ultra-fp4"
  }
}
```
...
Built-in models (varth-dev) have tier and description baked in. External models need metadata — see below.
...
Varth tags every LLM request with a `phase:{name}` label for usage analytics and cost attribution. The orchestrator sets the phase as work progresses and it is displayed in the footer.
...
Varth supports tagging LLM requests for usage tracking and cost attribution. Tags are included with every request and displayed in the footer, grouped by key with color coding.
...
Set via the `VARTH_TAGS` environment variable (comma-separated). Static tags are read-only within the session and shown with a `[static]` marker.

```bash
export VARTH_TAGS="team:backend,project:api"
```
...
User-defined tags (added via `/tags add`) are persisted to `~/.config/varth/tags.json` and survive across sessions. Static tags from `VARTH_TAGS` must be set each session.

## Ferment -- cross-session project management

Ferment is Varth's progressive-refinement project mode for multi-session work. Instead of starting from scratch each chat, Ferment persists a structured plan (goal, phases, steps) as a JSON state file.

### Quick start

```bash
varth --ferment "Build Tetris"
```
...
varth ships with built-in Language Server Protocol (LSP) support, giving the agent type-aware code intelligence. The extension loads by default — no configuration required.
...
Launch with `varth --teleport` to enable session-multiplex commands. The local TUI stays the home base; remote workers are spawned, detached, and re-attached without restarting varth.
...
1. `VARTH_API_KEY` environment variable (takes precedence)
2. `~/.config/varth/config.json` field `api_key`

Run `varth setup` for an interactive first-time configuration.

### Agent config

Varth stores its configuration (settings, sessions, models) under:

```
~/.config/varth/harness/
```

### Context files

You can provide custom instructions that are injected into the system prompt on every session. Varth discovers two kinds of context files:

**Global** — applied to every session, regardless of project:

```
~/.config/varth/harness/AGENTS.md
```
...
**Project-level** — applied when working in a specific directory tree. Varth walks from the working directory up to the filesystem root and collects one context file per directory:
...
Varth supports native Pi packages. `varth install npm:<package>` installs a package only for Varth, while `pi install npm:<package>` keeps the package owned by the original Pi harness. Varth can also load original Pi packages through the **Pi package lookup** resource, so both CLIs can use Pi packages without sharing Varth's install scope.

If the same package is installed in both places, the Varth install wins. Disable original Pi package lookup with:

```bash
varth resources disable extensions.pi-package-lookup
```

Update packages with:

```bash
varth update                  # update installed packages, then Varth itself
varth update --extensions     # update installed packages only
varth update context-mode     # update one package by source or display name
varth update self             # update Varth itself only
```

### HTTP proxy

Varth respects `HTTP_PROXY` / `HTTPS_PROXY` environment variables for network requests.

### Token optimization (RTK)

Varth installs [RTK](https://github.com/rtk-ai/rtk) during setup and keeps the `rtk` command available on startup. When enabled, varth rewrites bash tool calls through `rtk rewrite` before execution. This compresses command output (git, cargo, npm, docker, etc.) by 60-90%, reducing LLM context usage.

Before every bash tool execution, varth calls `rtk rewrite "<command>"`. If RTK returns a rewritten command (e.g. `git status` becomes `rtk git status`), the rewritten version is executed instead.
...
```bash
varth resources disable hooks.rtk-rewrite
varth resources enable hooks.rtk-rewrite
```

### Hooks

Users can add custom Bash hooks to rewrite or block shell commands before they run. Global hooks live in `~/.config/varth/harness/hooks/bash/`; project hooks live in `.varth/hooks/bash/` and default to disabled until enabled from `/resources` or `varth resources enable ...`.

See `docs/hooks.md` for the hook protocol and examples.

### Migrating from another coding agent

On first run, varth looks for an existing **Claude Code**, **OpenCode**, or **Cursor** installation and offers to migrate its MCP servers. If anything is migratable you will see a one-shot prompt:
...
|
*  Migrate MCP servers to Varth?
|  * Migrate now
|  * Skip this time
|  * Never ask again
```

Discovered MCP servers are merged into `~/.config/varth/harness/mcp.json`. Existing Varth entries always win on name collisions, so re-running the migration is safe.
...
3. **Against existing Varth config**: your entries in `~/.config/varth/harness/mcp.json` always win.

#### "Never ask again"

Stored in `~/.config/varth/config.json` (`migrationState: "skip-forever"`). Delete that field to re-trigger the prompt. Adding support for another agent is a small change -- drop a new definition into `src/agent-discovery/agents/` and append it to the registry.
...
```bash
git clone git@github.com:getvarth/varth.git
cd varth
...
The `benchmark/` directory contains tools for smoke-testing varth sessions and auditing their quality.
...
- **Terminal-bench-2** (`benchmark/terminal-bench-2/`) -- run the [terminal-bench](https://www.harborframework.com/) suite (89 tasks) against varth inside Docker containers. See `benchmark/terminal-bench-2/README.md`.
...
Release assets follow the naming convention `varth_{os}_{arch}.tar.gz` with a `checksums.txt` (SHA256) for verification.
