 ...
 *    checking that the approved plan file is written to .varth/plans/ when
	...
	async ({ terminal }) => {
		await runVarthSession(
			...
			async (fixture, trace) => {
				...
				const fermentsDir = join(fixture.workDir, ".varth", "ferments")
...
// <!-- PLAN_COMPLETE -->, the handler writes the approved plan to .varth/plans/
...
test("plan-to-ferment promotion — side effects: plan file written + tool swap at turn boundary", async ({
	...
	await runVarthSession(
		...
		async (fixture, trace) => {
			...
			// the approved plan to .varth/plans/ and transitions to auto mode.
			...
			const plansDir = join(fixture.workDir, ".varth", "plans")
