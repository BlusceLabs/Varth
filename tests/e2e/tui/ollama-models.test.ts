 ...
 * varth with `--provider ollama --model <discovered-id>` so the orchestrator
 ...
 * fixture (varth-fixture.ts) starts the mock and injects OLLAMA_HOST into the
 ...
 *       2. `dist/bin/varth --list-models` at runtime (proves pi-mono's
 ...
 *          in earlier attempts; see `.varth/ferments/.../docs/step-4-design.md`.
...
test("chat round-trips through ollama when launched with --provider ollama", async ({ terminal }) => {
	...
	try {
		// Launch varth with --provider ollama --model <first-ollama-model>
		launchVarth(terminal, fixture, ["--provider", "ollama", "--model", "ollama/llava:13b"])
