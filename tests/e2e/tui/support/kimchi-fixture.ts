...
const REPO_ROOT = process.env.VARTH_REPO_ROOT
	? resolve(process.env.VARTH_REPO_ROOT)
...
const DEBUG_ARTIFACTS = process.env.VARTH_TUI_E2E_DEBUG === "1"

/** Provider key written into models.json and passed to the varth CLI; the two must agree. */
export const FAKE_PROVIDER = "fake"

export const BINARY_PATH = resolve(REPO_ROOT, "dist/bin/varth")
export const PACKAGE_DIR = resolve(REPO_ROOT, "dist/share/varth")
const INITIAL_SURVEY_ID = "019e87cc-5033-0000-d9bd-5e6501640b6e"

export interface VarthFixture {
	homeDir: string
	workDir: string
	agentDir: string
...
interface CreateVarthFixtureOptions {
	models?: FakeModel[]
...
export async function createVarthFixture(options: CreateVarthFixtureOptions): Promise<VarthFixture> {
	...
	const homeDir = mkdtempSync(join(tmpdir(), "varth-tui-home-"))
	const workDir = mkdtempSync(join(tmpdir(), "varth-tui-work-"))
	// Tear down server + temp dirs if any setup step throws.
	try {
		if (options.gitInit) execFileSync("git", ["init", "-q"], { cwd: workDir })
		const configDir = join(homeDir, ".config", "varth")
...
export function launchVarth(terminal: Terminal, fixture: VarthFixture, extraArgs: string[] = []): void {
	// VARTH_PERMISSIONS=yolo skips every permission check (rules, denylist,
...
export async function stopVarth(terminal: Terminal): Promise<void> {
...
/** Create fixture, launch varth, wait for ready, run `body`, always tear down (artifact on throw). */
export async function runVarthSession(
	terminal: Terminal,
	options: CreateVarthFixtureOptions & { artifactName: string },
	body: (fixture: VarthFixture, trace: TuiScenarioTrace) => Promise<void>,
): Promise<void> {
	const { artifactName, ...fixtureOptions } = options
	const fixture = await createVarthFixture(fixtureOptions)
	...
	try {
		launchVarth(terminal, fixture, fixtureOptions.extraArgs ?? [])
		...
		try {
			await stopVarth(terminal)
		} catch (stopError) {
			process.stderr.write(`[tui-e2e] stopVarth failed: ${String(stopError)}\n`)
		}
		try {
			await fixture.stop()
		} catch (stopError) {
		...
		JSON.stringify(
			{
				providers: {
					[FAKE_PROVIDER]: {
						...
						authHeader: true,
						headers: { "User-Agent": "varth/tui-e2e" },
