...
import { PROMPT_READY, TUI_TEST_CONFIG, runVarthSession } from "./support/varth-fixture.js"
...
test("/multi-model metadata editor saves tier/vision/description", async ({ terminal }) => {
	await runVarthSession(
		...
		async (fixture, trace) => {
			...
			// default orchestrator is "varth-dev/kimi-k2.7" and it has
			// builtin metadata, so the wizard will offer "keep current (X)"
			// options alongside the explicit choices.
			await waitForText(terminal, "varth-dev/kimi-k2.7", { timeoutMs: INPUT_TIMEOUT_MS })
			...
			expect(viewText(terminal)).toContain("varth-dev/kimi-k2.7")
			terminal.submit("")
			// The submenu title is `${ref} — metadata`. Matching the full
			// `varth-dev/kimi-k2.7 — metadata` string avoids matching the
			// picker's own "Choose a model to edit metadata" header.
			await waitForText(terminal, "varth-dev/kimi-k2.7 — metadata", { timeoutMs: INPUT_TIMEOUT_MS })
			...
			await waitForText(terminal, /Metadata saved for varth-dev\/kimi-k2\.7\./, { timeoutMs: INPUT_TIMEOUT_MS })
			...
			const settingsPath = `${fixture.homeDir}/.config/varth/harness/settings.json`
			const settings = await readSettingsJson(settingsPath)
			const meta = settings.modelMetadata?.["varth-dev/kimi-k2.7"]
...
test("/multi-model builder role opens toggle-select with current assignment pre-checked", async ({ terminal }) => {
	await runVarthSession(
		...
		async (_fixture, trace) => {
			...
			expect(view).toMatch(/\[x\]\s+varth-dev\//)
