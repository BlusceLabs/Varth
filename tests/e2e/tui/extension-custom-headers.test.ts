...
import { TUI_TEST_CONFIG, runVarthSession } from "./support/varth-fixture.js"
 ...
 * `x-varth-tui-test-marker`. Telemetry's own `X-Session-Id`/`X-Turn-Index`
...
test("extension before_provider_headers hook injects custom headers into LLM requests", async ({ terminal }) => {
	...
	const extensionPath = join(fixtureRoot, `varth-tui-headers-${marker}.js`)
...
\t\t\t"X-Varth-Tui-Test-Marker": ${JSON.stringify(marker)},
\t\t});
\t});
};
`,
	...
	try {
		await runVarthSession(
			terminal,
			{
				...
				responses: [{ stream: ["Hello", " from", " fake", " Varth."] }],
			...
			async (fixture, trace) => {
				...
				await expect(terminal.getByText("Hello from fake Varth.", { full: true })).toBeVisible()
				...
				const headerValue = chatRequests[0].headers["x-varth-tui-test-marker"]
