...
import type { VarthFixture } from "./support/varth-fixture.js"
 ...
 */
function anyRequestContainsNudgePhrase(fixture: VarthFixture, phrase: string): boolean {
...
function anyRequestContainsAnyNudge(fixture: VarthFixture): boolean {
	return (
 ...
 * the happy path.
 */
async function waitForTurnToSettle(fixture: VarthFixture) {
 ...
 */
async function waitForFirstRequest(fixture: VarthFixture, timeoutMs = 10_000) {
	const startedAt = Date.now()
	while (Date.now() - startedAt < timeoutMs) {
