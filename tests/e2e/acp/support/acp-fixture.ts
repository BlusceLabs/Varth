// ACP integration test fixture. Spawns varth with `--mode acp`, returns a
...
const BINARY_PATH = resolve(REPO_ROOT, "dist/bin/varth")
const PACKAGE_DIR = resolve(REPO_ROOT, "dist/share/varth")
...
/** Time to wait for varth to start and accept the first JSON-RPC frame. */
...
export interface AcpFixtureOptions {
	 ...
	 * into the `_varth.dev/pi_*` extension namespace via per-method flags.
	 */
	clientMeta?: Record<string, unknown>
	/**
	 * Path to a JavaScript extension to drop into the harness's
	 * `extensions/` directory before spawning varth. Defaults to the
...
export async function startAcpFixture(options: StartAcpFixtureOptions): Promise<AcpFixture> {
	...
	const homeDir = mkdtempSync(join(tmpdir(), "varth-acp-home-"))
	const workDir = mkdtempSync(join(tmpdir(), "varth-acp-work-"))
	...
	try {
			...
			JSON.stringify(
				{
					providers: {
						fake: {
							...
							authHeader: true,
							headers: { "User-Agent": "varth/acp-e2e" },
							models: [
								{
									id: "basic",
									name: "Fake Basic",
		...
		proc = spawn(BINARY_PATH, ["--mode", "acp"], {
			...
			env: {
				...
				VARTH_DISABLE_BUILTIN_PROVIDERS: "1",
		...
		// If varth crashes on startup this rejects within the timeout.
...
function mergeCapabilities(
	...
	// that's where `_meta["varth.dev"].pi_*` capability checks look for it.
	return {
		...override,
		fs: {
			readTextFile: false,
