// ACP integration: client advertises every `_varth.dev/pi_*` method and
...
// Spread the varth source of truth so this stays in sync when a method is added.
const PI_META = { "varth.dev": { ...ADVERTISED_CAPABILITIES } } as const
...
describe("ACP integration — all capabilities", () => {
					...
					toolCalls: [
						{
							function: {
								...
								arguments: JSON.stringify({
									command: "touch /tmp/varth-acp-marker-all.txt",
								}),
							},
						},
					],
	...
	it("drives a text reply, a tool call, and a permission request through the full ACP surface", async () => {
		...
		const t2 = await prompt(
			fixture,
			sessionId,
			"Use the bash tool to run exactly `touch /tmp/varth-acp-marker-all.txt` and reply with the word done",
		)
		...
		expect(
			fixture.client.hasNotification("_varth.dev/pi_notify"),
			`_varth.dev/pi_notify extNotification arrived (saw: ${fixture.client.extNotifications.map((n) => n.method).join(", ")})`,
		).toBe(true)
		expect(
			fixture.client.hasNotification("_varth.dev/pi_setStatus"),
			"_varth.dev/pi_setStatus extNotification arrived",
		).toBe(true)
