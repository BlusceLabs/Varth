...
// `_varth.dev/pi_*` method. Expected: basic ACP flows still work; tool-call
...
describe("ACP integration — no capabilities", () => {
					...
					toolCalls: [
						{
							function: {
								name: "bash",
								arguments: JSON.stringify({ command: "touch /tmp/varth-acp-marker-none.txt" }),
							},
						},
					],
				},
	...
	it("drives text + tool calls + permissions even with no UI capabilities advertised", async () => {
		...
		const t2 = await prompt(
			fixture,
			sessionId,
			"Use the bash tool to run exactly `touch /tmp/varth-acp-marker-none.txt` and reply with the word done",
		)
		...
		const piNotifications = fixture.client.extNotifications.filter((n) => n.method.startsWith("_varth.dev/pi_"))
		expect(piNotifications, "no _varth.dev/pi_* extNotifications").toEqual([])
		...
		expect(notifyWarning, "notify warning references missing capability").toContain("_varth.dev/pi_notify")
		expect(setStatusWarning, "setStatus warning references missing capability").toContain("_varth.dev/pi_setStatus")
	})
})
