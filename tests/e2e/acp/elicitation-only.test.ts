...
// `_varth.dev/pi_*` methods. Expected: basic ACP flows (text, tool calls,
...
describe("ACP integration — elicitation only", () => {
	...
	beforeEach(async () => {
		fixture = await startAcpFixture({
					...
					toolCalls: [
						{
							function: {
								...
								arguments: JSON.stringify({
									command: "touch /tmp/varth-acp-marker-elicit.txt",
								}),
							},
						},
					],
			...
			// No clientMeta — the client did NOT advertise any _varth.dev/pi_* methods.
		})
	})
	...
	it("surfaces fire-and-forget UI calls as agent_message_chunk warnings instead of dropping them", async () => {
		...
		const t2 = await prompt(
			fixture,
			sessionId,
			"Use the bash tool to run exactly `touch /tmp/varth-acp-marker-elicit.txt` and reply with the word done",
		)
