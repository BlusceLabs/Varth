	...
	describe("compound bash in rpc mode", () => {
						...
						toolCalls: [
							{
								function: {
									...
									arguments: JSON.stringify({
										command: "echo a && touch /tmp/varth-acp-marker-compound.txt",
									}),
								},
							},
						],
		...
		it("issues a single permission card for compound bash (not per-subcommand)", async () => {
			...
			const t2 = await prompt(
				fixture,
				sessionId,
				"Use the bash tool to run exactly `echo a && touch /tmp/varth-acp-marker-compound.txt` and reply with the word done",
			)
	...
	describe("deny-with-feedback", () => {
						...
						toolCalls: [
							{
								function: {
									name: "bash",
									arguments: JSON.stringify({
										command: "touch /tmp/varth-acp-marker-deny.txt",
									}),
								},
							},
						],
		...
		it("collects free-text feedback via elicitation when the user rejects a permission", async () => {
			...
			const t2 = await prompt(
				fixture,
				sessionId,
				"Use the bash tool to run exactly `touch /tmp/varth-acp-marker-deny.txt` and reply with the word done",
			)
			expect(t2.stopReason, "turn 2 stop reason").toBe("end_turn")
