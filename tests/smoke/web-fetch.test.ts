...
describe("web_fetch smoke tests", () => {
	it.skipIf(!process.env.VARTH_API_KEY)("web_fetch tool is registered and available", { retry: 2 }, () => {
	...
	it.skipIf(!process.env.VARTH_API_KEY)("fetches a web page via the web_fetch tool", { retry: 2 }, () => {
		const result = runBinary({
			args: [
				"--debug-prompts",
				"-p",
