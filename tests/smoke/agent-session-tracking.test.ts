// End-to-end checks that a real varth run spawning a real Agent leaves parent
...
describe("Agent session tracking smoke tests", () => {
	...
	beforeEach(() => {
		sessionDir = mkdtempSync(join(tmpdir(), "varth-agent-session-"))
	})
	...
	it.skipIf(!process.env.VARTH_API_KEY)(
