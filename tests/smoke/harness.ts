 ...
 * varth binary with a sandboxed HOME. The binary computes its agent
 * config dir as HOME/.config/varth/harness, so we expose that derived
...
export const BINARY_PATH = resolve("dist/bin/varth")
export const PACKAGE_DIR = resolve("dist/share/varth")

let tempHome: string | undefined

beforeAll(() => {
	tempHome = mkdtempSync(join(tmpdir(), "varth-smoke-home-"))
	const configDir = join(tempHome, ".config", "varth")
	...
	// the dummy VARTH_API_KEY gets a 401 from the live metadata endpoint. Without
	// this, interactive smoke tests would fail to boot the binary in CI.
	const agentDir = join(tempHome, ".config", "varth", "harness")
 ...
 * Matches the logic in src/cli.ts: resolve(homedir(), ".config", "varth", "harness").
 */
export function getAgentDir(): string {
	return join(getTempHome(), ".config", "varth", "harness")
}
...
export interface InteractiveSession {
	...
	pty: IPty
	/** Everything varth has printed so far, ANSI escapes intact. */
	output(): string
	...
	/** Type `data` into varth as if from a keyboard. */
...
/** Spawn varth under a real PTY so its interactive TUI boots. Prefer `runBinary` for non-interactive checks — use this only when you need to drive the editor/prompt. */
export function spawnInteractive(opts: RunInteractiveOptions = {}): InteractiveSession {
	...
	const pty = ptySpawn(BINARY_PATH, [], {
		...
		env: {
			...
			PI_PACKAGE_DIR: PACKAGE_DIR,
			TERM: "xterm-256color",
			VARTH_API_KEY: "smoke-test-dummy",
			...extraEnv,
		},
	})
