 ...
 * varth harness.
 *
 * Requires VARTH_API_KEY to be set (skipped otherwise).
...
describe("web_search smoke tests", () => {
	it.skipIf(!process.env.VARTH_API_KEY)("web_search tool is registered and available", { retry: 2 }, () => {
	...
	it.skipIf(!process.env.VARTH_API_KEY)("web_search returns results for a factual query", { retry: 2 }, () => {
		const result = runBinary({
			args: [
				"--debug-prompts",
				"-p",
