...
// Wait for the interactive prompt to be fully mounted. The editor placeholder "ask anything or type / for commands" renders once the Editor is mounted. The startup hint line is suppressed by varth's forced `quietStartup: true`, so we can't probe for "clear/exit" text. First-time spawns on a cold sandbox HOME download fd/rg (~30s), so the timeout is generous.
