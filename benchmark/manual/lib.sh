...
#
# runner_type = "varth" | "claude"
# model_name: for varth type, pass "" to run in multi-model mode (no --model flag).
...
"
    ...
    if [[ -n "$model_name" ]]; then
      ...
      model_flag_line="  --model varth-dev/${model_name} \\
"
    fi
...
#   - 2D (rows=R, cols=C): R horizontal splits × C vertical splits — for varth grid
