...
```json
{
  "binary": "/path/to/your/varth",
  "models": ["kimi-k2.5", "minimax-m2.7"]
}
```

- `binary` — path to the varth binary (`~` is expanded)
...
- **Single-model** — `--model varth-dev/<model>` forces one model for the entire session. Used by `complex-single`, generated once per model in `benchmark.json`.
...
This launches `varth` in `--yolo` mode with the self-improvement prompt. The agent will cycle through build → benchmark → analyse → code change phases automatically, stopping when a stopping condition is met (see `self-improvement.md` for details).
