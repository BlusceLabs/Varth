...
# the varth harness with kimi-k2.6.
#
# Usage:
...
for task in "${TASKS[@]}"; do
  audit_cmd="cd \"$REPO_ROOT\" && \"$AUDIT_SCRIPT\" -m varth-dev/claude-opus-4-6 -s \"${AUDIT_SESSION_FILES["$task"]}\" \"${JSONL_FILES["$task"]}\""
...
echo "Audit reports should be in: ${REPO_ROOT}/.varth/audits/"
echo "========================================"