...
Audits a completed varth harness session, analyzing phase discipline, code quality, model alignment, and cost efficiency. Produces a graded report written to `.varth/audits/`.
...
```sh
# Interactive session picker, default runner (varth)
...
# Custom model
./benchmark/audit-session/audit-session.sh -m varth-dev/kimi-k2.6
...
| `-r`, `--runner CMD` | Harness to use: `varth` (default) or `claude` |
| `-m`, `--model MODEL` | Model override (default: `varth-dev/claude-opus-4-6` for varth, `claude-opus-4-6` for claude) |
| `-h`, `--help` | Show help |
...
|--------|------|-------------|---------------|
| `varth` | interactive, yolo | all tools allowed | `varth-dev/claude-opus-4-6` |
...
The audit report is written to `.varth/audits/{sessionId}-{task}-{runner}-{model}-AUDIT.md` in the project directory. It includes:
...
```
.varth/audits/session-20260511-124841-complex-AUDIT.md
```
...
4. Launches the chosen runner (varth or claude-code) in interactive mode with the prompt
...
| File | Description |
|------|-------------|
| `.varth/audits/{sessionId}-{task}-{runner}-{model}-AUDIT.md` | Human-readable graded report |
| `.varth/audits/{sessionId}-{task}-{runner}-{model}-AUDIT.json` | Machine-readable sidecar for automated comparison |
...
jq -s '.[0].aggregates.totalCost as $a | .[1].aggregates.totalCost as $b |
  ...
  .varth/audits/session-AUDIT.json .varth/audits/session-BUDIT.json
...
jq -s 'map(.aggregates.totalCost) | add' .varth/audits/*-AUDIT.json
```

## Prerequisites

- `varth` and/or `claude` CLI available on PATH
- `jq` recommended (used for session parsing and JSON sidecar validation; falls back to grep)
- Session JSONL files (from `~/.config/varth/harness/sessions/` or `benchmark/manual/`)
