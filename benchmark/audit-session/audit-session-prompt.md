...
7. Read project context files (if they exist in the session's cwd):
   - README.md
   - CLAUDE.md
   - .varth/ directory contents
```

For production code evidence:
```
8. Find new/modified source files produced during this session:
    git log --oneline --since="<session_start_timestamp>" -- ':!.gsd/' ':!.varth/'
    git diff --stat <first_session_commit>..HEAD -- ':!.gsd/' ':!.varth/'
...
Write the audit report to: `.varth/audits/{auditFilename}`

Create the `.varth/audits/` directory if it does not exist.
...
Write a new file at `.varth/audits/{auditFilename}` containing:
...
After writing the JSON sidecar, say: "Audit complete for session {sessionId}. Report at .varth/audits/{auditFilename}"
