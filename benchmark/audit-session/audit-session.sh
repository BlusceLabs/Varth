...
SESSIONS_BASE="${HOME}/.config/varth/harness/sessions"
...
Audit a varth harness session for phase quality and cost efficiency.
...
Options:
  ...
  -r, --runner CMD    Harness to use: varth or claude (default: varth)
  -m, --model MODEL   Model to use (default: varth-dev/claude-opus-4-6)
  -s, --session PATH  Write session JSONL to PATH (varth only)
  -h, --help          Show this help

Examples:
  $(basename "$0")                              # pick session, run with varth
...
case "$RUNNER" in
    varth)  EFFECTIVE_MODEL="${MODEL:-varth-dev/claude-opus-4-6}" ;;
    claude)  EFFECTIVE_MODEL="${MODEL:-claude-opus-4-6}" ;;
    *)       echo "Unknown runner: $RUNNER (use 'varth' or 'claude')" >&2; exit 1 ;;
esac
...
echo "Audit file: .varth/audits/$AUDIT_FILENAME"
echo ""
...
run_audit_agent() {
    ...
    case "$runner" in
        varth)
            local -a varth_args=(--model "$model" --yolo)
            if [[ -n "$SESSION_OUT" ]]; then
                varth_args+=(--session "$SESSION_OUT")
            fi
            varth_args+=("@${prompt_file}")
            varth "${varth_args[@]}"
            ;;
        claude)
...
echo ""
audit_file=".varth/audits/$AUDIT_FILENAME"
