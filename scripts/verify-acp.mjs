#!/usr/bin/env node
// Scripted ACP client that drives varth --mode acp through handshake, newSession, prompt.
...
async function main() {
	...
	try {
		...
		// Tool-call test: prompt varth to invoke a tool and verify tool_call + tool_call_update flowed through.
		...
		// global timeout. /tags is a varth extension command (src/extensions/tags.ts) so it
