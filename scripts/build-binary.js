...
run(
	"compile",
	`bun build src/entry.ts --compile${targetFlag} --compile-exec-argv="--use-system-ca" --outfile dist/bin/varth --external chromium-bidi --external electron`.trim(),
)
...
if (!isCrossCompile && platform() === "darwin") {
	run("codesign (strip)", "codesign --remove-signature dist/bin/varth")
	run("codesign (ad-hoc)", "codesign -s - dist/bin/varth")
}
