...
//
// Both scenarios drive a real LLM via stdio JSON-RPC; VARTH_API_KEY must be
// in env. Companion to verify-acp.mjs / verify-acp-stop.mjs.
//
// Usage: node scripts/verify-acp-load.mjs [path-to-varth-binary]
...
const binary = process.argv[2] ?? "./dist/bin/varth"
...
async function scenarioCrossToolRoundTrip() {
	...
	// CLI leg: varth reopens the same session via --session <id> and adds turn 2.
	...
	process.stderr.write(`[B2] launching varth --session ${sessionId}\n`)
