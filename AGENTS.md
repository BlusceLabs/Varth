# Agent Guidelines for Varth-Dev

You are editing the varth-code CLI harness. This repo extends the pi-mono SDK (`@earendil-works/pi-coding-agent`) — core agent loop lives upstream; this repo adds extensions in `src/extensions/`.
...
- **Structure TUI tests through the shared fixture.** Use `runVarthSession`, deterministic fake OpenAI responses, isolated temp `HOME`/workdir, and trace steps for meaningful checkpoints. Avoid brittle ANSI/snapshot assertions unless the rendering itself is the behavior under test.
...
## Documents Directory
- `.varth/docs/` → Transient AI working files — git-ignored, do NOT commit
